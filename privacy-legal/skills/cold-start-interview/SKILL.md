...
description: >
  ...
  me", "configure privacy", or wants to re-run the interview or re-check integrations.
  WorkBuddy
  中国语境适配：默认中国大陆法域，用于数据合规与个人信息保护场景下的初始化访谈。中文触发词包括：中国法、中国合规、数据合规与个人信息保护、初始化访谈、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**数据合规与个人信息保护**；当前技能：**初始化访谈**。
...
1. Check `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` — if populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.workbuddy/skills/cache/workbuddy-cn-legal/privacy-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` (create parent directories as needed). Show summary. Offer first task.

## `--check-integrations`

Re-runs the integration availability check (document storage, 企业微信/飞书/钉钉, scheduled-tasks) and updates `## Available integrations` in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `~/.workbuddy/skills/cache/workbuddy-cn-legal/privacy-legal/*/CLAUDE.md` but not at the config path, copy it forward.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/privacy-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /privacy-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> Who'll be using this plugin day to day? (This feeds every skill's work-product header and output framing — lawyer gets "律师工作底稿/内部法律分析," non-lawyer gets research framing and attorney-review checkpoints before legally consequential steps.)
>
...
> This plugin can work with: document storage (飞书云文档/企业微信文档/钉钉文档/Google Drive, SharePoint), 企业微信/飞书/钉钉, and scheduled-tasks. Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
| 企业微信/飞书/钉钉 | [✓ / ✗] | Breach / triage notifications delivered inline instead of posted |
...
2. **Research connector prompt.** Say:
   ...
   > "Before your first DPA review or PIA: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In WorkBuddy: authorize when a skill prompts you."
...
5. **Close with the "you can change anything later" note:**
   ...
   > "Your practice profile is at `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
   >
