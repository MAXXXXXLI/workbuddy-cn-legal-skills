...
description: >
  ...
  comes in, the user pastes an access/deletion/portability/correction request, or says
  "DSAR came in", "access request", "right to be forgotten", or "someone wants their
  data". WorkBuddy
  中国语境适配：默认中国大陆法域，用于数据合规与个人信息保护场景下的个人信息权利请求响应。中文触发词包括：中国法、中国合规、数据合规与个人信息保护、个人信息权利请求响应、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**数据合规与个人信息保护**；当前技能：**个人信息权利请求响应**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → DSAR process (systems list, verification method, SLA).
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## DSAR process`. That section has:
...
Per the method in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. Common approaches:
...
Walk the systems list from `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. For each system:
...
**Before proceeding to send either letter to the data subject:** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
> **Note:** Both DSAR letters are externally-facing deliverables sent to the data subject. Do **not** include the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` `## Outputs` on either letter. Internal notes, logs, and exemption analyses that accompany the letters are 律师工作底稿/内部法律分析（中国法下不等同于美国工作成果保护） — keep those separate and prepend the work-product header per `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` `## Outputs` (which differs by user role — see `## Who's using this`).

> **Before sending either letter:** This is a draft for attorney review, not a response to send. Sending commits the controller to a position, may waive exemptions, and may start a regulator's clock. A 执业律师/法务负责人 reviews, edits, and approves before either letter goes to the data subject. Do not send unreviewed.
...
Per `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → Escalation table, escalate when:
...
If `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## DSAR process` records an internal SLA that is tighter than the legal deadline, use the internal SLA and note the legal backstop.
