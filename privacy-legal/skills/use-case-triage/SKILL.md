...
description: >
  ...
  "is this okay from a privacy perspective", or describes a new data processing activity,
  product feature, or vendor relationship. WorkBuddy
  中国语境适配：默认中国大陆法域，用于数据合规与个人信息保护场景下的用例分级。中文触发词包括：中国法、中国合规、数据合规与个人信息保护、用例分级、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**数据合规与个人信息保护**；当前技能：**用例分级**。
...
1. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. Confirm privacy practice is configured — if not, stop and direct to setup.
...
Before triaging, always read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. The PIA trigger criteria, regulatory
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## PIA house style` → Trigger criteria. Apply them.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## Privacy policy commitments`. Check the proposed activity
