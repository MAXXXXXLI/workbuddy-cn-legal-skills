...
description: >
  ...
  DPA", "is this DPA okay", or attaches a DPA. WorkBuddy
  中国语境适配：默认中国大陆法域，用于数据合规与个人信息保护场景下的数据处理协议审查。中文触发词包括：中国法、中国合规、数据合规与个人信息保护、数据处理协议审查、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**数据合规与个人信息保护**；当前技能：**数据处理协议审查**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → DPA playbook. If placeholders, stop and prompt setup.
...
DPAs come in two flavors and the review is nearly opposite for each. When a customer sends their DPA, we're defending our operational flexibility. When we send one to a vendor, we're protecting our (and our customers') data. Both reviews read from the same `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` playbook but from opposite rows.

## First: which direction?

Before anything else, establish:

- **We are the processor** → customer is sending us their DPA → read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → "When we are the processor" table
...
Before reviewing, check the outputs folder for prior work on this counterparty or processing activity. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## Outputs` for the outputs folder path. Scan for:
...
**Carry severity from the upstream output as a floor** per the cross-skill severity floor rule in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## Shared guardrails`. A processing activity the triage rated 🔴 cannot be quietly downgraded to 🟢 in the DPA review; any demotion is stated and explained.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## DPA playbook`. Also read `## Privacy policy commitments` — the DPA can't contradict what the privacy policy promises.
...
Walk every DPA through these terms, clause by clause. The *specific* numeric and substantive positions (notice periods, breach timelines, acceptable/unacceptable floors) come from `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## DPA playbook`. The regulatory floors that any DPA has to clear come from primary law — **research the currently operative rule** for each applicable regime and cite primary sources before stating a floor.
...
Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
**Before proceeding to sign or countersign a DPA (including returning an executed version, consenting to automatic execution on a counterparty platform, or instructing a signatory to execute):** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
