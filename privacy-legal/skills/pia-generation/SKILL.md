...
description: >
  ...
  "privacy review this feature", or describes a new data processing activity. WorkBuddy
  中国语境适配：默认中国大陆法域，用于数据合规与个人信息保护场景下的个人信息保护影响评估。中文触发词包括：中国法、中国合规、数据合规与个人信息保护、个人信息保护影响评估、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**数据合规与个人信息保护**；当前技能：**个人信息保护影响评估**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → PIA house style (trigger, structure, depth, sign-off).
...
Before writing a new PIA, check the outputs folder for prior work on the same feature, processing activity, or counterparty. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## Outputs` for the path. Scan for:
...
**Carry severity from upstream as a floor** per the cross-skill severity floor rule in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## Shared guardrails`. A use-case-triage that rated the activity high-risk cannot become a PIA that concludes low-risk without stating why and what changed.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → `## PIA house style`. That has:
...
Check the trigger criteria in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. That is the team's house answer.
...
**Use the seed PIA structure from the config CLAUDE.md.** If none was captured, use this default. Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
Every PIA should cross-check against the privacy policy commitments in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. The common drift:
...
- **To the sign-off process:** Per `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → who approves PIAs.
...
**Before proceeding to submit a DPIA (or any equivalent impact assessment) to a regulator, supervisory authority, or enforcement body:** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
