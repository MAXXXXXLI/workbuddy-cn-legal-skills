...
本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**数据合规与个人信息保护**；当前技能：**政策漂移监测**。
...
1. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → outputs folder path, policy document, last sweep date.
...
6. Update Last policy sweep date in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`.

**Direct query mode** (with description argument):
1. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → current policy commitments + actual policy document.
...
Update **Last policy sweep** in `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` to today's date after completing the sweep.
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md`, so the next sweep only looks at new files.
...
- It doesn't enforce that outputs are saved — if the team isn't saving PIAs to the
  ...
  saved outputs.
- It doesn't read email or 企业微信/飞书/钉钉 for informal decisions — only structured outputs
