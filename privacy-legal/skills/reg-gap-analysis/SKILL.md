...
本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**数据合规与个人信息保护**；当前技能：**监管差距分析**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → privacy policy commitments, regulatory footprint, DSAR systems.
...
This skill diffs the new requirement against what you currently do (per `~/.workbuddy/skills/config/workbuddy-cn-legal/privacy-legal/CLAUDE.md` → Privacy policy commitments + the practices documented in PIAs) and produces a gap list with a remediation plan.
