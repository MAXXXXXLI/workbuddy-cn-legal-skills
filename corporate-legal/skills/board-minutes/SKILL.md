...
description: >
  ...
  minutes. Also handles written consents in lieu of meetings. Trigger: "board minutes",
  "draft minutes", "upcoming board meeting", "committee minutes", "written consent", or
  calendar detection of an upcoming board or committee event. WorkBuddy
  中国语境适配：默认中国大陆法域，用于公司与交易法务场景下的董事会/股东会会议记录。中文触发词包括：中国法、中国合规、公司与交易法务、董事会/股东会会议记录、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**公司与交易法务**；当前技能：**董事会/股东会会议记录**。
...
- `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` → `## Board & Secretary` section:
...
- If `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` has no minutes format: run cold-start first. Do not proceed with a generic format.
...
**Directors present:**
- Pull from board composition in `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` as the starting point
...
Use the house format from `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`. Do not default to a generic format. The seed minutes are the template — replicate the structure, the header, the resolution language, the level of discussion detail.

### Standard structure (adapt to house format)

**Header block:**
```
MINUTES OF [法考主观题TING TYPE] OF THE BOARD OF DIRECTORS
...
**Previous minutes:**
Standard language: approval of minutes from prior meeting. Pull date of prior meeting from `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` board calendar if available, otherwise leave as [DATE OF PRIOR 法考主观题TING].
...
RESOLVED, that [resolution text in house language from `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`].
```
...
**Before adopting minutes as final:** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
...
Produce the full draft. The minutes themselves are a corporate record, not privileged; do not apply the work-product header to the minutes as circulated. The drafting notes, placeholder flags, and review checklist below are work product — prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`):
...
> This is a draft for attorney review, not adopted minutes. Adopted minutes are the official record of board action and carry legal consequences — a 执业律师/法务负责人 reviews, edits, and takes professional responsibility before adoption. Do not adopt this draft unreviewed.
