...
description: >
  ...
  writes only the relevant sections to the plugin config. Use on fresh install, when
  CLAUDE.md still has [PLACEHOLDER] markers, when starting a new deal, or to re-check
  integrations or refresh a module. WorkBuddy
  中国语境适配：默认中国大陆法域，用于公司与交易法务场景下的初始化访谈。中文触发词包括：中国法、中国合规、公司与交易法务、初始化访谈、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**公司与交易法务**；当前技能：**初始化访谈**。
...
1. Check `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`. If `--new-deal`, skip to per-deal setup. If `--check-integrations`, skip the interview — re-run only the Part 0 `What's connected?` check and rewrite the `## Available integrations` table in `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`. When probing: only report ✓ if an MCP tool call actually succeeded. Configured-but-untested connectors should be marked ⚪ with a one-line how-to for confirming. Never report ✓ based on `.mcp.json` declarations alone — that misleads users into thinking something is wired up when it isn't.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.workbuddy/skills/cache/workbuddy-cn-legal/corporate-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` (create parent directories as needed). For `--new-deal`, write `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/deal-context.md`.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `~/.workbuddy/skills/cache/workbuddy-cn-legal/corporate-legal/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/corporate-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /corporate-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> This plugin can work with: VDR (Intralinks, Datasite, 企业网盘/Box), board portal (Diligent, BoardEffect), document storage, and 企业微信/飞书/钉钉. Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
- What's your usual VDR — Intralinks, Datasite, 企业网盘/Box, SharePoint, something else?
...
- How do you brief the deal team — daily, weekly, milestone-based? Email, 企业微信/飞书/钉钉, call?
...
>
> If you have one: tell me where it lives (folder path, 飞书云文档/企业微信文档/钉钉文档/Google Drive folder, SharePoint library, 企业网盘/Box folder). The skill will search it at runtime.
>
...
- What are the key jurisdictions — just 公司注册地/章程约定地（中国公司须按《公司法》和章程核验）, or a meaningful multi-jurisdiction footprint?
...
> "Before your first diligence extraction or consent: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In WorkBuddy: authorize when a skill prompts you."
...
> "Your practice profile is at `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
>
...
Write to `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code-name]/deal-context.md`. Skills read both the plugin config (house) and `deal-context.md` (this deal), with deal-context.md taking precedence on conflicts.
