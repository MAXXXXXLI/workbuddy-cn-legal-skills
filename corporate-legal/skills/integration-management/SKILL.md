...
本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**公司与交易法务**；当前技能：**并购后整合管理**。
...
5. Read/write `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/integration-tracker.yaml`.
...
Lives at `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/integration-tracker.yaml`. Read `deal-context.md` for
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/deal-context.md`. If not found: ask for deal code name,
target company, close date, deal lead, and outside counsel. Write to
deal-context.md if it doesn't exist.

Read `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/closing-checklist.yaml` if it exists. Any items marked as
...
>   automatically from `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/closing-checklist.yaml`
...
> Is your contract repository connected? (飞书云文档/企业微信文档/钉钉文档/Google Drive, 企业网盘/Box, SharePoint,
...
**Manual update:** Attorney tells WorkBuddy what changed.
...
WorkBuddy updates the relevant tracker entry, recalculates any downstream status
