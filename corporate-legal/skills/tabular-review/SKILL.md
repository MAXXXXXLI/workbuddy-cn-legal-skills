...
description: >
  ...
  needs a spreadsheet out the other end. Use when user says "tabular review", "review
  grid", "build a grid", "extract these fields from these contracts", "review these
  documents for X, Y, Z", "give me a spreadsheet of", "batch review", or points at a
  folder of documents and asks to compare them. WorkBuddy
  中国语境适配：默认中国大陆法域，用于公司与交易法务场景下的表格式尽调审查。中文触发词包括：中国法、中国合规、公司与交易法务、表格式尽调审查、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**公司与交易法务**；当前技能：**表格式尽调审查**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` → diligence structure, thresholds, house format.
...
- `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` → diligence structure, materiality thresholds, house format preferences
- `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/deal-context.md` if working a specific deal
...
Confirm:
1. **Documents.** Where are they? VDR MCP (企业网盘/Box, Datasite, iManage), local folder, 飞书云文档/企业微信文档/钉钉文档/Google Drive folder, or a list of files. How many? If >200, warn that this will take a while and offer to start with a materiality-filtered subset.
