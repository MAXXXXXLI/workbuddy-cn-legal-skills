...
description: >
  ...
  VDR documents. WorkBuddy
  中国语境适配：默认中国大陆法域，用于公司与交易法务场景下的尽调问题提取。中文触发词包括：中国法、中国合规、公司与交易法务、尽调问题提取、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**公司与交易法务**；当前技能：**尽调问题提取**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` + `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/deal-context.md`.
...
The VDR has 2,000 documents. Somewhere in there are the 30 that matter for the deal. This skill reads documents against the diligence categories and materiality thresholds from `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`, extracts issues, and writes them in house memo format.

## Load context

- `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` → Diligence structure (categories, materiality thresholds)
- `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` → Issues memo format (how findings are stated)
- `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/deal-context.md` → deal-specific thresholds, VDR location
...
If VDR MCP (企业网盘/Box/Intralinks/Datasite) is connected, pull the index. Map VDR folders to diligence request list categories. Note gaps — request list categories with no corresponding VDR content.
...
Per `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` / deal-context thresholds. Don't review everything if the threshold says contracts >$X.
...
Per the finding template in `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`. If the seed memo used this:
...
- **To ai-tool-handoff:** If Luminance/Kira is in use per `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`, hand bulk contract review there. This skill handles the nuanced documents (side letters, amendments, anything the AI tool struggles with).
...
- It doesn't replace bulk AI review. For high-volume clause extraction, hand off to Luminance/Kira per `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`. This skill is for the judgment layer.
