...
description: >
  ...
  in the next 30/60/90 days. Use when user says "entity compliance", "filing deadlines",
  "annual reports due", "entity tracker", "what filings are due", "entity health", or
  "good standing". WorkBuddy
  中国语境适配：默认中国大陆法域，用于公司与交易法务场景下的主体合规。中文触发词包括：中国法、中国合规、公司与交易法务、主体合规、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**公司与交易法务**；当前技能：**主体合规**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` → `## Entity Management` (entity table, jurisdictions, registered agent).
...
3. Read/write `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/entities/compliance-tracker.yaml`.
...
is a file you own, WorkBuddy updates it on command, and you export it when you need
to share it.
...
Lives at `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/entities/compliance-tracker.yaml`. Structure:
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` → `## Entity Management` → Entity table. If the entity table
...
Generate `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/entities/compliance-tracker.yaml` with all entities and their
...
**Before directing or confirming a filing:** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
...
Attorney tells WorkBuddy what was filed:
> "We filed the 公司注册地/章程约定地（中国公司须按《公司法》和章程核验） annual report for [Entity] on March 1. Fee was $450."
...
report (PDF, CSV, or Excel). WorkBuddy reads it and updates matching entities:
...
- Based on `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` entity table: are there states in the company's
  ...
  WorkBuddy can flag the question but cannot determine presence independently.

**Intercompany agreement gaps:**
- From `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`: if intercompany agreements are marked as partial or no,
...
POTENTIAL GAPS
  Foreign qualification: [flag question — confirm operational presence in:]
    [list of states from `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` footprint not in tracker as qualified]
  Intercompany agreements: [status from `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md`]
...
a report or 企业微信/飞书/钉钉 message, showing only the next 90 days of filings.
