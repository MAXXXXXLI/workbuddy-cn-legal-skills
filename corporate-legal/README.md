...
Walks through module selection, then a short targeted interview for each active area. Writes a modular `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` with only the relevant sections. Your configuration is stored at that path and survives plugin updates.
...
**Connect a research tool first — the citation guardrails depend on it.** Without one, every cite is tagged `[verify]` and the reviewer note above each deliverable records that sources weren't verified. Skills work either way; a research tool (人民法院案例库/中国裁判文书网) just shifts verification work off your plate.

Ships with:

- **企业微信/飞书/钉钉** — search messages, read channels, find discussions (general bucket)
- **飞书云文档/企业微信文档/钉钉文档/Google Drive** — search, read, and fetch documents (general bucket)
- **企业网盘/Box** — data room and document management

Intralinks, Datasite, and other VDR connectors can be added to `.mcp.json` when partner URLs are available.

## How it learns

Your practice profile at `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. You can re-run setup, edit the file directly, or tell a skill to record a new position.
...
- AI tool handoff (Luminance/Kira) is optional. If `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` says no tool, all extraction runs through the direct skill.
