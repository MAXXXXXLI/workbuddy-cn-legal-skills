...
Daily during active diligence. Checklist status per `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` → Deal team briefing cadence.

## Integrations

Posting to 企业微信/飞书/钉钉 requires a 企业微信/飞书/钉钉 MCP server in your environment. This plugin does not bundle one. If no 企业微信/飞书/钉钉 MCP is configured, write the VDR update and checklist status to a file in `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/updates/[date].md` and notify the user — do not fail silently.

VDR tools (企业网盘/Box, Intralinks, Datasite) are likewise external MCPs — if none are connected, prompt the user for the VDR export or ask them to update `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/vdr-inventory.md` manually.
