...
4. On first run after a plugin update, if a populated CLAUDE.md exists at the old cache path
   (~/.workbuddy/skills/cache/workbuddy-cn-legal/corporate-legal/<version>/CLAUDE.md for any version)
...
本配置模板已从 Claude 法律插件转换为 WorkBuddy 中国语境版本。模块：**公司与交易法务**。
...
|---|---|---|
| VDR (Intralinks, Datasite, 企业网盘/Box) | [✓ / ✗] | Diligence pulls from local folder; user drops docs in `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/deals/[code]/vdr-mirror/` |
| Board portal (Diligent, BoardEffect) | [✓ / ✗] | Minutes/consents work from local templates; no portal posting |
| Document storage (飞书云文档/企业微信文档/钉钉文档/Google Drive, SharePoint, 企业网盘/Box) | [✓ / ✗] | Read local paths; no cross-system search |
| 企业微信/飞书/钉钉 | [✓ / ✗] | Briefs emitted as files only; no in-channel summaries |
...
- If Role is **Non-lawyer** (either type): `RESEARCH NOTES — NOT LEGAL ADVICE — REVIEW WITH A 执业律师/法务负责人, SOLICITOR, BARRISTER, OR OTHER AUTHORISED LEGAL PROFESSIONAL IN YOUR JURISDICTION BEFORE ACTING`
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/verification-log.md`:
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/CLAUDE.md` (and `~/.workbuddy/skills/config/workbuddy-cn-legal/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
...
**VDR typical:** [PLACEHOLDER — Intralinks / Datasite / 企业网盘/Box / SharePoint / varies]
...
**Format:** [PLACEHOLDER — email / 企业微信/飞书/钉钉 / call]
...
**Consents repository:** [PLACEHOLDER — folder path / Google 飞书云文档/企业微信文档/钉钉文档/SharePoint location, or "seed documents only"]
**Consent format:**
...
**Compliance tracker:** `~/.workbuddy/skills/config/workbuddy-cn-legal/corporate-legal/entities/compliance-tracker.yaml`
