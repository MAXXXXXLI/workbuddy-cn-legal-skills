...
name: "产品合规法务-上线审查"
description: >
  用于中国大陆产品合规法务场景下的上线审查。输出默认简体中文；正式依赖前需法务负责人或执业律师核验。
  Full launch review against your framework and risk calibration. Use when the user says
  "review this launch", "legal review for [feature]", "can we ship this", "what are the
  legal issues with [product]", or references a launch tracker ticket or PRD that needs a
  category-by-category review memo. WorkBuddy
  中国语境适配：默认中国大陆法域，用于产品合规法务场景下的上线审查。中文触发词包括：中国法、中国合规、产品合规法务、上线审查、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**产品合规法务**；当前技能：**上线审查**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md` → framework + calibration. Stop if placeholders.
...
Read the PRD, check every category in this team's framework, calibrate against what actually blocks here (per `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md`), and output a review in house format. Goal: a PM reads it and knows exactly what has to happen before they ship.
...
For each category in `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md` → Review framework. If the team doesn't have one, use the 8-category default below. The categories are stable framing concepts; within each category, research the regulatory regimes applicable to the product's sector, audience, and jurisdictions before calibrating severity. What blocks in one jurisdiction or sector may be routine in another — `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md` captures the team's calibration.
...
> **No silent supplement.** If a research query to the configured legal research tool (北大法宝/威科先行/官方法规库, 人民法院案例库/中国裁判文书网, regulator sites, or firm platform) returns few or no results for a regime, enforcement precedent, or regulator guidance, report what was found and stop. Do NOT fill the gap from web search or model knowledge without asking. Say: "The search returned [N] results from [tool]. Coverage appears thin for [regime / topic]. Options: (1) broaden the search query, (2) try a different research tool, (3) search the web — results will be tagged `[web search — verify]` and should be checked against the issuing authority before relying, or (4) flag as unverified and stop. Which would you like?" A lawyer decides whether to accept lower-confidence sources.
>
...
| **Gaming / loot boxes / in-game currency** | Loot-box odds disclosure (CA AB 2476-style, Chinese / Korean / Belgian / Dutch regimes), ESRB / PEGI descriptors (In-Game Purchases, Loot 企业网盘/Boxes, Real Gambling), state gambling law (games-of-chance vs. games-of-skill lines, sweepstakes promotions law), FTC dark-patterns guidance, platform-store policies (Apple, Google, console) |
...
For each finding, check against the calibration table in ~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md:
...
Format per `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md` → Launch review process → output format. Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`). If no house format is specified:
...
> **Before emitting a "Clear to ship" or "Ship with conditions" call:** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md`. If the Role is Non-lawyer:
>
...
**Output 1 — Privileged launch review memo.** The full analysis assembled in Step 5: work-product header, bottom line, findings by category with risk rationale, action items, escalations, notes for next time, citation check. This is internal legal work product. Keep it in your matter file (Drive, DMS, or wherever `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md` says review docs go). Distribute only to people inside the privilege circle.
...
- It doesn't retroactively calibrate. If this launch turns out fine (or badly) in a way that should update the calibration table, a human updates ~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md.
