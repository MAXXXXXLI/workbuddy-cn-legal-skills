...
name: "产品合规法务-事项工作区"
description: >
  用于中国大陆产品合规法务场景下的事项工作区。输出默认简体中文；正式依赖前需法务负责人或执业律师核验。
  Manage matter workspaces — new, list, switch, close, or detach (practice-level). Use
  when working across multiple clients or matters in private practice and you need to
  create, list, switch, close, or detach the active matter so context from one engagement
  doesn't leak into another. WorkBuddy
  中国语境适配：默认中国大陆法域，用于产品合规法务场景下的事项工作区。中文触发词包括：中国法、中国合规、产品合规法务、事项工作区、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**产品合规法务**；当前技能：**事项工作区**。
...
```
~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/
