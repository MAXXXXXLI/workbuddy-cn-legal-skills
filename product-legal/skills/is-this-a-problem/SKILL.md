...
description: >
  ...
  that needs a same-minute fine / needs a look / hold call. WorkBuddy
  中国语境适配：默认中国大陆法域，用于产品合规法务场景下的快速合规判断。中文触发词包括：中国法、中国合规、产品合规法务、快速合规判断、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**产品合规法务**；当前技能：**快速合规判断**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md` → Risk calibration.
...
Most "quick legal question" 企业微信/飞书/钉钉s are one of three things: (a) not a problem, say so fast, (b) a real thing that needs a real look, route it, (c) a thing that looks fine but has a trap, catch the trap. This skill sorts in under a minute using the calibration table.

The goal is speed. The PM asked at 4:47pm. They want an answer, not a memo.

## Load calibration

Read `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md` → `## Risk calibration`. The whole point of this skill is pattern-matching against that table.
...
**For 企业微信/飞书/钉钉 (the common case):**

企业微信/飞书/钉钉 triage replies are internal legal advice. If the reply is being pasted into a ticket, document, or channel that's broadly shared with non-legal, prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/product-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`):

```
[WORK-PRODUCT HEADER — per plugin config ## Outputs]
```

For an in-the-flow 企业微信/飞书/钉钉 DM reply to the PM, the short form is:
