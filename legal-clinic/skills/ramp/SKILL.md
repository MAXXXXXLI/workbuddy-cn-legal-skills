...
本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**法律诊所/法律援助**；当前技能：**诊所上手培训**。
...
1. Check `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` is set up. If placeholders: "Ask [professor] to run `/legal-clinic:cold-start-interview` first."
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → clinic profile, practice areas, jurisdiction, handbook path, supervision style, practice-area templates.
...
Read from `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` and the ingested handbook. Cover, interactively:
...
- What WorkBuddy can help with / what it can't (starting points yes, final work product no, authoritative citations no)
