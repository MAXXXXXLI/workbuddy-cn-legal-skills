---
name: "法律诊所/法律援助-初始化访谈"
description: >
  用于中国大陆法律诊所/法律援助场景下的初始化访谈。输出默认简体中文；正式依赖前需法务负责人或执业律师核验。
  Professor's one-time clinic setup — practice areas, jurisdiction, supervision style
  (formal review queue / configurable flags / lighter-touch), and handbook/rules upload.
  Writes CLAUDE.md so every other skill and every student who runs /ramp reads from the
  same clinic context. Use on fresh install, when CLAUDE.md has placeholders, when
  re-doing setup with --redo, or when re-checking integrations with --check-integrations.
  WorkBuddy
  中国语境适配：默认中国大陆法域，用于法律诊所/法律援助场景下的初始化访谈。中文触发词包括：中国法、中国合规、法律诊所/法律援助、初始化访谈、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**法律诊所/法律援助**；当前技能：**初始化访谈**。
...
1. Check `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md`. If populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.workbuddy/skills/cache/workbuddy-cn-legal/legal-clinic/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` including `## Who's using this` and `## Available integrations`. Show supervision choice and practice-area templates for confirmation.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md`:
...
- **Pause and resume.** Tell the supervising attorney up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/legal-clinic:cold-start-interview` again later and I'll pick up where you left off." When the attorney pauses, write a partial configuration to `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /legal-clinic:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the attorney: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
1. **Account tier and data-handling terms.** Your WorkBuddy account tier and its data retention and training policies — Team, Enterprise, Work, Education, and individual accounts have different guarantees about retention, use for training, and subprocessor handling. Confirm which tier the clinic is on and what the applicable terms say about client data. Document the answer in the plugin config.

2. **Client consent and disclosure practices for AI-assisted work.** Review ABA Formal Opinion 512 (2024), your 司法行政机关/律师协会's AI guidance (if any), and Model Rules of Professional Conduct 1.1 (competence), 1.4 (communication), 1.6 (confidentiality), and 5.3 (supervision of nonlawyer assistance). Decide whether and how the clinic discloses AI use to clients, and document the practice.
...
4. **Practice-area heightened-confidentiality considerations.** Immigration, criminal defense, domestic violence, family, and some civil rights matters carry heightened confidentiality and security expectations that go beyond the baseline — adversary exposure risk, 法院/仲裁机构调查取证或协助调查文件 risk, safety risk for survivors. Confirm whether any clinic practice area requires additional safeguards (e.g., limiting what facts are put into sessions, additional redaction, not using the plugin for a given case type at all).
...
> This plugin can work with a case management system (Clio) and document storage (飞书云文档/企业微信文档/钉钉文档/Google Drive, SharePoint, 企业网盘/Box). Let me check which connectors are configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
Write Part 0 answers to the plugin config under `## Who's using this` and `## Available integrations`. If a populated CLAUDE.md exists at the old cache path `~/.workbuddy/skills/cache/workbuddy-cn-legal/legal-clinic/*/CLAUDE.md` but not here, copy it forward first.
...
- **Ethical reminders calibrated to task:** e.g., /draft outputs remind about 中国法律诊所/法律援助监督要求 supervision requirements
...
6. **Before your first case review, connect a research tool.** Say: "Before your first case review or memo: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In WorkBuddy: authorize when a skill prompts you."
...
> Done. Your clinic's configuration is at `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
