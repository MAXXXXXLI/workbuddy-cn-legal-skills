...
description: >
  ...
  intake, running an intake interview, or writing up a new client's situation. WorkBuddy
  中国语境适配：默认中国大陆法域，用于法律诊所/法律援助场景下的客户接谈。中文触发词包括：中国法、中国合规、法律诊所/法律援助、客户接谈、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**法律诊所/法律援助**；当前技能：**客户接谈**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → practice areas, intake templates, supervision style, flag triggers.
...
**What it doesn't do:** decide whether to take the case. That's the student's analysis and the professor's judgment. WorkBuddy accelerates the information-gathering and structuring, not the lawyering.

## Load context

`~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → practice areas, intake templates (per practice area if multiple), supervision style, jurisdiction, flag triggers.

## Read the supervisor guide

Check for a practice-area guide at `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/guides/<practice-area>.md`. If one exists, use its intake questions, red flags, and good-fit criteria instead of the generic defaults below. If one doesn't exist, use the generic intake and note at the end of the intake summary: "This was a generic intake — your supervisor can tailor the questions for your clinic type with `/legal-clinic:build-guide`."
...
Each practice area asks different questions. Use the template from `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` for this area. Defaults if none provided:
...
Per whatever conflict-check process `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` describes. At minimum:
...
Per `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` supervision style and flag triggers. If formal queue or configurable flags are enabled, and a trigger is present (deadline mentioned, DV indicator, immigration status at issue, etc.), note the flag.
