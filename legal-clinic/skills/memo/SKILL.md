...
description: >
  ...
  their analysis, or build an IRAC memo for a case. WorkBuddy
  中国语境适配：默认中国大陆法域，用于法律诊所/法律援助场景下的法律备忘录。中文触发词包括：中国法、中国合规、法律诊所/法律援助、法律备忘录、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**法律诊所/法律援助**；当前技能：**法律备忘录**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → practice areas, jurisdiction.
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → practice areas, jurisdiction, supervision style.
