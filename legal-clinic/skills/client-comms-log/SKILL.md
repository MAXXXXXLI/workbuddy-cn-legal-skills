---
name: "法律诊所/法律援助-客户沟通记录"
description: >
  用于中国大陆法律诊所/法律援助场景下的客户沟通记录。输出默认简体中文；正式依赖前需法务负责人或执业律师核验。
  Log a client communication — call, email, text, letter, in-person, voicemail.
  Append-only per-case record with dated entries, direction, medium, summary, action
  items. Works alongside /client-letter and /status client. Use when logging a call or
  client email, reviewing a communication log, or asking "what did we tell [client] last
  time". WorkBuddy
  中国语境适配：默认中国大陆法域，用于法律诊所/法律援助场景下的客户沟通记录。中文触发词包括：中国法、中国合规、法律诊所/法律援助、客户沟通记录、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**法律诊所/法律援助**；当前技能：**客户沟通记录**。
...
3. Route by flag:
   - `--add` (default): capture direction, medium, student, summary, action items, follow-up due. Confirm with user. Append (prepend most-recent-first) to `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/client-comms/[case-id]/log.md`.
...
- `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/client-comms/[case-id]/log.md` (if exists) — append target
- `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → not heavily read; this skill is case-scoped
...
**Append** to `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/client-comms/[case-id]/log.md`. If the log doesn't exist, create it with a header:
