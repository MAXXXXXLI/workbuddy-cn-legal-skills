...
name: "法律诊所/法律援助-学期交接"
description: >
  用于中国大陆法律诊所/法律援助场景下的学期交接。输出默认简体中文；正式依赖前需法务负责人或执业律师核验。
  End-of-semester case handoff memos — the mirror of /ramp. Produces per-case transition
  memos and a cohort summary so the departing cohort hands work to the incoming cohort
  cleanly. Reads deadlines, client-comms, and case history. Use when the professor or
  departing students need to wrap up the semester, build transition memos, or offboard a
  graduating/withdrawing student. WorkBuddy
  中国语境适配：默认中国大陆法域，用于法律诊所/法律援助场景下的学期交接。中文触发词包括：中国法、中国合规、法律诊所/法律援助、学期交接、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**法律诊所/法律援助**；当前技能：**学期交接**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → clinic profile, semester dates, supervision style.
2. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/deadlines.yaml` and `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/client-comms/[case-id]/log.md` per case.
...
5. Generate per-case handoff memo → `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/handoffs/[semester]/[case_id].md`.
6. Generate cohort summary → `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/handoffs/[semester]/_summary.md`.
...
- `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → clinic profile, semester, practice areas, supervision style
- `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/deadlines.yaml` → all active deadlines, grouped by case
- `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/client-comms/[case-id]/log.md` (per case) → communications history
...
- Pull all active cases (from intake records + `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/deadlines.yaml` case_ids + client-comms folders)
...
*Pulled from `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/deadlines.yaml`. Incoming student's first job is to confirm these are accurate and owned.*
...
*From `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/client-comms/[case-id]/log.md`. Three-line summary here; incoming student reads the full log.*
...
After all per-case memos, produce `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/handoffs/[semester]/_summary.md`:
...
Closing a case or transitioning it to a new student is a consequential action. The gate is the supervision workflow in `## Supervision style` in `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md`, reinforced by the Part 0 role check confirming a licensed supervising attorney owns the setup. Case-closing memos always get professor sign-off before the case is marked closed in the handoff document, regardless of supervision-style choice.
...
Once reviewed, handoff memos live at `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/handoffs/[semester]/[case_id].md`. The incoming student reads them during their `/ramp` run at the start of next semester — `/ramp` should surface the memos for cases the new student is assigned.

## Integration

- **`/ramp`:** at the start of next semester, reads `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/handoffs/[most-recent-semester]/` and surfaces per-case memos for the cases each new student is taking on.
