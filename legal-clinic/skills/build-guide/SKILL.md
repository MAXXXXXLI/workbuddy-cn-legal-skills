...
description: >
  ...
  build or revise a per-practice-area guide, tune how the clinic skills behave for their
  clinic type, or set their teaching philosophy as plugin configuration. WorkBuddy
  中国语境适配：默认中国大陆法域，用于法律诊所/法律援助场景下的诊所搭建指南。中文触发词包括：中国法、中国合规、法律诊所/法律援助、诊所搭建指南、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**法律诊所/法律援助**；当前技能：**诊所搭建指南**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → role (must be Supervising attorney), practice areas, jurisdiction.
...
5. Write `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/guides/<practice-area>.md`. Create the `guides/` directory if needed.
...
This is a supervisor skill. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` → `## Who's using this` → Role. If the role is not "Supervising attorney," say:
...
If a guide already exists at `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/guides/<practice-area>.md`, offer: "A guide for [area] already exists at [path]. Do you want to (a) revise it section-by-section, (b) start fresh and overwrite, or (c) see what's there first?"
...
- **Housing clinic:** `litigation-legal:法院/仲裁机构调查取证或协助调查文件-triage` when the client brings in a 法院/仲裁机构调查取证或协助调查文件, wrapped so the student drafts the response plan but the supervisor signs off.
...
Write to `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/guides/<practice-area>.md`. Create the `guides/` directory if it doesn't exist. Use this structure:
...
The skill's "output" is the file written at `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/guides/<practice-area>.md`. The conversation with the supervisor is the interview; the written guide is the artifact.
