# WorkBuddy for Law School Clinics
...
| `/research-start [issue]` | Research roadmap: statutes, case law areas, 北大法宝/威科先行/官方法规库 search terms | **Leads, not authoritative citations** — students verify everything |
...
1. **Your WorkBuddy account tier and its data retention and training policies.** Team, Enterprise, Work, Education, and individual accounts have different guarantees about retention, training use, and subprocessor handling. Confirm what applies to the clinic's account.
2. **Your client consent and disclosure practices for AI-assisted work** per ABA Formal Opinion 512 (2024), your 司法行政机关/律师协会's AI guidance (if any), and Model Rules 1.1, 1.4, 1.6, and 5.3. Decide whether and how the clinic discloses AI use to clients; document it.
...
Changeable later by editing `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md`. Your configuration is stored at that version-independent path and survives plugin updates.
...
`/ramp --card` generates the one-page student reference card: commands, what WorkBuddy can and can't help with, verification habits. Hand it out on day one.
...
Your practice profile at `~/.workbuddy/skills/config/workbuddy-cn-legal/legal-clinic/CLAUDE.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. You can re-run setup, edit the file directly, or tell a skill to record a new position.
