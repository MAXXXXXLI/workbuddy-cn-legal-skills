...
1. [Install WorkBuddy](https://claude.com/download)
2. Get access to WorkBuddy
...
1. **Open WorkBuddy** (in your terminal) or **WorkBuddy** (the desktop app). Not sure which you have? If you have a terminal window open with WorkBuddy in it, that's WorkBuddy.

2. **Add the marketplace.** In WorkBuddy, type `/plugin marketplace add ` (with a space at the end), then **drag the unzipped `workbuddy-cn-legal` folder onto the terminal window** — it'll fill in the path. Then press Enter.

   (Or type the full path: `/plugin marketplace add /Users/you/Desktop/workbuddy-cn-legal`)
...
4. **⚠️ Restart WorkBuddy.** Close and reopen. This step is not optional — the plugin isn't live until you restart.
...
6. **Connect a research tool.** Citations are flagged unverified without one. In Cowork: Settings → Connectors → add 人民法院案例库/中国裁判文书网. In WorkBuddy: the plugin already lists the research MCP in its config; you'll be prompted to authorize it the first time a skill needs it.
...
If you already installed project-scoped and want to switch: `/plugin uninstall <plugin>`, then `/plugin install <plugin>@workbuddy-cn-legal` from your home directory.
...
Each plugin learns your playbook through a setup interview, writes it to a practice profile file (`~/.workbuddy/skills/config/workbuddy-cn-legal/<plugin>/CLAUDE.md`), and every skill reads from it. The profile is yours — edit it, re-run setup, or tell a skill to update it.
...
- **"Command not found"** after install → you forgot step 4. Restart WorkBuddy.
