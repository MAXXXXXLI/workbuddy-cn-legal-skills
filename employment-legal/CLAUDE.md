...
4. On first run after a plugin update, if a populated CLAUDE.md exists at the old cache path
   (~/.workbuddy/skills/cache/workbuddy-cn-legal/employment-legal/<version>/CLAUDE.md for any version)
...
本配置模板已从 Claude 法律插件转换为 WorkBuddy 中国语境版本。模块：**劳动用工法务**。
...
|---|---|---|
| HRIS (Workday, BambooHR, Rippling, ADP) | [✓ / ✗] | Leave data tracked in `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/leave-register.yaml`; manual entry via `/employment-legal:log-leave` |
| Document storage (飞书云文档/企业微信文档/钉钉文档/Google Drive, SharePoint, 企业网盘/Box) | [✓ / ✗] | Read local paths for handbook + seed documents |
| 企业微信/飞书/钉钉 | [✓ / ✗] | Reviews emitted as files only; no in-channel summaries |
...
- `[人民法院案例库/中国裁判文书网]` / `[statute / regulator site]` / `[user provided]` — where a cite actually came from. Provenance, not confidence. Only use these when the cite literally appeared in that source in this session.
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/verification-log.md`:
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` (and `~/.workbuddy/skills/config/workbuddy-cn-legal/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
...
**Leave data access:** [Legal has read access / manual — `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/leave-register.yaml`]
