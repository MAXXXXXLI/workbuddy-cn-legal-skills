...
本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**劳动用工法务**；当前技能：**劳动关系/用工形态识别**。
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` → jurisdictional footprint, any classification history or
...
If `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` records the company's house classification policy, apply it
...
Escalate per `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` if any of the following, or any team-specific
...
**Before producing a "Proceed as IC / employee / agency / vendor" final recommendation:** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
