...
description: >
  ...
  this offer", "can we use a non-compete here", "check this offer letter", "hiring in
  [state]", or attaches an offer. WorkBuddy
  中国语境适配：默认中国大陆法域，用于劳动用工法务场景下的招聘录用审查。中文触发词包括：中国法、中国合规、劳动用工法务、招聘录用审查、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**劳动用工法务**；当前技能：**招聘录用审查**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` → jurisdictional footprint, hiring review triggers, restrictive covenant policy.
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` → jurisdictional footprint, hiring review triggers, restrictive
covenant policy, offer letter template location.

## Output header

Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`).
...
Check the jurisdiction table in `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` for this state/country. If it's
...
Per `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` restrictive covenant policy: does this hire even get one?
...
Check the `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` table for this jurisdiction. Common categories to
...
**Before producing a "Clear to send" recommendation or a final offer letter for signature:** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
