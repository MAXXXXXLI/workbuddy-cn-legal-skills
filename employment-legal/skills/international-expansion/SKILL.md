...
description: >
  ...
  /expansion-kickoff and /expansion-update; not invoked directly. WorkBuddy
  中国语境适配：默认中国大陆法域，用于劳动用工法务场景下的跨境/异地扩张。中文触发词包括：中国法、中国合规、劳动用工法务、跨境/异地扩张、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**劳动用工法务**；当前技能：**跨境/异地扩张**。
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` → jurisdictional footprint, escalation table, any existing
expansion notes.
...
Write a new file to `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/expansion-[country-slug].yaml` with all open items
...
**Roles:** [list]
**Tracker:** ~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/expansion-[slug].yaml
