...
description: >
  ...
  Loaded by /investigation-open, /investigation-add, /investigation-query,
  /investigation-memo, and /investigation-summary; not invoked directly. WorkBuddy
  中国语境适配：默认中国大陆法域，用于劳动用工法务场景下的内部调查。中文触发词包括：中国法、中国合规、劳动用工法务、内部调查、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**劳动用工法务**；当前技能：**内部调查**。
...
Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`). Every file, log, memo, and summary produced by this skill opens with that header.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/CLAUDE.md` → escalation table, any investigation protocols noted.
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/investigation-[matter-slug]/log.yaml`:
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/investigation-[matter-slug]/sources-checklist.yaml`:

Generated from the investigation type. See sources checklist templates below.

`~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/investigation-[matter-slug]/documents-reviewed.yaml`:
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/investigation-[slug]/sources-checklist.yaml`.
...
If multiple investigation folders exist in `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/`, ask which matter this
...
Write the draft to `~/.workbuddy/skills/config/workbuddy-cn-legal/employment-legal/investigation-[slug]/memo.md`.
