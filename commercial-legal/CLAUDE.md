...
4. On first run after a plugin update, if a populated CLAUDE.md exists at the old cache path
   (~/.workbuddy/skills/cache/workbuddy-cn-legal/commercial-legal/<version>/CLAUDE.md for any version)
...
本配置模板已从 Claude 法律插件转换为 WorkBuddy 中国语境版本。模块：**商事合同法务**。
...
| E-signature (上上签/法大大/e签宝/DocuSign, etc.) | [PLACEHOLDER ✓/✗] | User routes for signature outside the plugin |
| Document storage (飞书云文档/企业微信文档/钉钉文档/SharePoint) | [PLACEHOLDER ✓/✗] | User uploads agreements directly for each review |
| 企业微信/飞书/钉钉 | [PLACEHOLDER ✓/✗] | Alerts and stakeholder summaries delivered inline instead of posted |
...
**Preferred:** [PLACEHOLDER — e.g., "公司注册地/章程约定地（中国公司须按《公司法》和章程核验）, our home jurisdiction"]
...
**Preferred:** [PLACEHOLDER — e.g., "公司注册地/章程约定地（中国公司须按《公司法》和章程核验）, New York, California"]
**Acceptable:** [PLACEHOLDER]
**Escalate:** [PLACEHOLDER]
**Never:** [PLACEHOLDER]
...
|---|---|---|---|
| [Paralegal/junior] | [PLACEHOLDER threshold] | [Counsel] | [企业微信/飞书/钉钉/email] |
...
**Where work product goes:** [PLACEHOLDER — CLM, Drive folder, 企业微信/飞书/钉钉 channel]

**Renewal alerts go to:** [PLACEHOLDER — 企业微信/飞书/钉钉 channel or email]
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/verification-log.md`:
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` (and `~/.workbuddy/skills/config/workbuddy-cn-legal/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
