...
name: "商事合同法务-初始化访谈"
description: >
  用于中国大陆商事合同法务场景下的初始化访谈。输出默认简体中文；正式依赖前需法务负责人或执业律师核验。
  Run the cold-start interview to learn your commercial contracts practice and write your
  team practice profile. Use on first use of the plugin, when
  `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` is missing or
  still contains template placeholders, or when the user says "set up the plugin",
  "configure commercial contracts", "onboard me", or "let's get started". This is the only
  skill that should run on a fresh install. WorkBuddy
  中国语境适配：默认中国大陆法域，用于商事合同法务场景下的初始化访谈。中文触发词包括：中国法、中国合规、商事合同法务、初始化访谈、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**商事合同法务**；当前技能：**初始化访谈**。
...
Runs the cold-start interview. First run writes `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`; subsequent runs with `--redo` re-interview and show a diff before overwriting.

## Instructions

1. **Check current state:** Read `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`. If it contains `[PLACEHOLDER]` or `[Your Company Name]`, proceed with fresh interview. If populated and `--redo` not passed, ask: "Looks like you're already set up. Want to re-run the interview? This will overwrite `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` (I'll show you a diff first)."

2. **Follow the interview script below.**

3. **Ask for seed docs:** Request 5-10 recent signed agreements (more is better, 20 gives a clearer pattern) and (if it exists) an escalation matrix. Accept file paths, 飞书云文档/企业微信文档/钉钉文档/Google Drive links, or [CLM] record IDs.

4. **Read the seed docs** and extract actual playbook positions. Note deltas between stated positions and what was signed.

5. **Migration:** If a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.workbuddy/skills/cache/workbuddy-cn-legal/commercial-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.

6. **Write `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`** (create parent directories as needed) per the structure below. Use the lawyer's own words where possible.

7. **Show summary + propose next steps:**
   - "Here's what I heard — `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` is written. What did I get wrong?"
   - Offer a test review: "Want to throw a contract at me?"
   - If a [CLM] is connected: offer to bulk-load the renewal register

## `--check-integrations`

Re-runs the integration availability check (CLM, e-signature, document storage, 企业微信/飞书/钉钉) and updates `## Available integrations` in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
Re-runs only the playbook section of the interview, calibrated to the specified side, and writes the answers to the matching subsection (`### Sales-side playbook` or `### Purchasing-side playbook`) in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`. Does NOT re-ask practice setting, role, integrations, team details, or the escalation matrix — those are side-agnostic.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `~/.workbuddy/skills/cache/workbuddy-cn-legal/commercial-legal/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
Look for `~/.workbuddy/skills/config/workbuddy-cn-legal/company-profile.md`.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/commercial-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /commercial-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> Who'll be using this plugin day to day? (This feeds the work-product header on every /review, /amendment-history, and /renewals-due output — lawyer gets "保密 / 内部法律分析 — 律师工作底稿/内部法律分析"; non-lawyer gets "RESEARCH NOTES — NOT LEGAL ADVICE" plus research-framed outputs.)
>
...
> If you need to find an attorney, solicitor, barrister, or other authorised legal professional: contact your professional regulator (司法行政机关/律师协会 in the US, SRA/Bar Standards Board in England & Wales, Law Society in Scotland/NI/Ireland/Canada/Australia, or your jurisdiction's equivalent) — most offer a lawyer referral service (your jurisdiction's bar association, law society, or legal aid body) as the fastest starting point. Many offer free or low-cost initial consultations. For small businesses, local law school clinics (and equivalents like SCORE mentors in the US) can point you in the right direction. For individuals, legal aid organizations cover many practice areas.

#### What's connected?

> This plugin can work with: CLM (Ironclad, Agiloft, etc.), e-signature (上上签/法大大/e签宝/DocuSign, etc.), document storage (飞书云文档/企业微信文档/钉钉文档/Google Drive, SharePoint, 企业网盘/Box), and 企业微信/飞书/钉钉. Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "企业网盘/Box isn't connected. In WorkBuddy: Settings → Connectors → Add → 企业网盘/Box → sign in. In WorkBuddy: add the 企业网盘/Box MCP to your config or via `/mcp`. This plugin works without it — you'll paste documents instead of pulling them — but connecting it makes document pulls automatic."
...
**Who are you?**
- Company name and entity type (公司注册地/章程约定地（中国公司须按《公司法》和章程核验） C-corp? LLC? Something else?)
...
**Channel and timing**
- How do people escalate today — 企业微信/飞书/钉钉, email, a ticket, a standing meeting?
...
| E-signature (上上签/法大大/e签宝/DocuSign, etc.) | [✓ / ✗] | User routes for signature outside the plugin |
| Document storage (飞书云文档/企业微信文档/钉钉文档/SharePoint) | [✓ / ✗] | User uploads agreements directly for each review |
| 企业微信/飞书/钉钉 | [✓ / ✗] | Alerts and stakeholder summaries delivered inline instead of posted |
...
|---|---|---|---|
| [Junior] | [their threshold] | [You] | [企业微信/飞书/钉钉/email] |
...
**Where work product goes:** [[CLM]? 飞书云文档/企业微信文档/钉钉文档/Google Drive folder? 企业微信/飞书/钉钉 thread?]

**Where signed contracts live:** [CLM system + executed filter / 飞书云文档/企业微信文档/钉钉文档/Google Drive folder path / SharePoint library / manual upload]
...
- If Role is Lawyer / legal professional: `保密 / 内部法律分析 — 律师工作底稿/内部法律分析 — PREPARED AT THE DIRECTION OF COUNSEL`
- If Role is Non-lawyer: `RESEARCH NOTES — NOT LEGAL ADVICE — REVIEW WITH A 执业律师/法务负责人, SOLICITOR, BARRISTER, OR OTHER AUTHORISED LEGAL PROFESSIONAL IN YOUR JURISDICTION BEFORE ACTING`
...
2. **Research connector prompt.** Say:
   ...
   > "Before your first contract review: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In WorkBuddy: authorize when a skill prompts you."
...
5. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your practice profile is at `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
