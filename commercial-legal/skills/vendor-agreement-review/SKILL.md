...
description: >
  ...
  approver. Loaded by /commercial-legal:review when a vendor MSA, services agreement, or
  similar is detected. WorkBuddy
  中国语境适配：默认中国大陆法域，用于商事合同法务场景下的供应商合同审查。中文触发词包括：中国法、中国合规、商事合同法务、供应商合同审查、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**商事合同法务**；当前技能：**供应商合同审查**。
...
Read a vendor agreement against the playbook this team actually uses (in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`), find every term that deviates, and tell the lawyer what to do about each one — with specific redline language, not vague "consider revising."
...
**Before reading the contract, read `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`.** If it's missing or still has placeholders, surface this bounce:
...
The playbook in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` is the source of truth. It tells you:
...
Check the "one thing" from `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` first. If present:
...
For each playbook category in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`, find the corresponding contract section and compare.
...
**Playbook says:** [our standard position, quoted from `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`]
...
**If they won't move:** [the fallback from `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`, or "escalate to [person]"
...
| Level | Means |
|---|---|
| 🔴 Critical | Don't sign without fixing. A term on the team's "never accept" list in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`, or a deal-breaker. |
...
Severity is always applied *against `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`*. If a term doesn't map cleanly to a playbook position, ask the user which bucket it belongs in and offer to record the answer in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`.
...
Check the escalation matrix in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` against:
...
**Before proceeding to send redlines to the counterparty:** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
## Integration: 上上签/法大大/e签宝/DocuSign

If 上上签/法大大/e签宝/DocuSign MCP is connected and the agreement is ready to sign (all greens or all issues accepted), offer to:
...
**Before generating a signature envelope or routing for countersignature:** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Full memo (default):** As above. Goes in the [CLM] record or the Drive folder from `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` house-style section.

**企业微信/飞书/钉钉-sized summary:** Two lines and a link. For when someone asks "is this okay?" in a channel.
...
- [ ] `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` was loaded and quoted — not generic market positions
