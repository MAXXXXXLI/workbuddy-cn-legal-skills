...
本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**商事合同法务**；当前技能：**业务摘要**。
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` → `## House style` → who reads stakeholder summaries, how long should they be. If not specified, default to: procurement or a department head, two paragraphs max, no legal terms of art.
...
**To the stakeholder:** Via whatever channel `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` says. If 企业微信/飞书/钉钉, keep it under 150 words. If email, the format above is fine as-is.
