...
description: >
  ...
  renewal tracker", or on a scheduled basis. Receives handoffs from saas-msa-review.
  WorkBuddy
  中国语境适配：默认中国大陆法域，用于商事合同法务场景下的续约跟踪。中文触发词包括：中国法、中国合规、商事合同法务、续约跟踪、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**商事合同法务**；当前技能：**续约跟踪**。
...
1. **Read `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/renewal-register.yaml`** (the config directory — survives plugin updates).
...
Lives at `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/renewal-register.yaml` (the config directory — survives plugin updates). Each entry:
...
2. Query 上上签/法大大/e签宝/DocuSign for completed envelopes in the last 24 months with "subscription" / "renewal" / "auto-renew" in metadata
...
**Before proceeding to accept or decline a renewal (including sending a non-renewal notice or letting an auto-renewal run past the cancel-by date):** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
The renewal-watcher agent in this plugin runs this skill on a schedule (weekly by default) and posts the "coming up" report to the channel named in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` → `## House style` → where work product goes. Mode 2 is the agent's primary output.
