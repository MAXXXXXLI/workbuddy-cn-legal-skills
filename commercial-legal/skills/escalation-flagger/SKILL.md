...
description: >
  ...
  need GC sign-off", "route this for approval", or when another skill finds an issue that
  exceeds the reviewer's authority. WorkBuddy
  中国语境适配：默认中国大陆法域，用于商事合同法务场景下的升级审批提示。中文触发词包括：中国法、中国合规、商事合同法务、升级审批提示、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**商事合同法务**；当前技能：**升级审批提示**。
...
Names the approver for a contract issue per the `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` escalation matrix and drafts the message so you're not writing "hey got a sec" at 5pm.

## Instructions

1. **Load `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`** → Escalation section. If missing, say so — the practice profile needs editing.
...
Every contracts team has an escalation matrix, written or not. This skill reads the written one (in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`), matches a contract issue against it, names the approver, and drafts the ask so the lawyer isn't writing "hey do you have a sec" messages at 5pm.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` → `## Escalation`. If it's missing or vague, say so — the cold-start interview should have captured this, and if it didn't, the practice profile needs editing.

Expected structure:

| Can approve | Threshold | Escalates to | Via |
|---|---|---|---|
| Paralegal | Standard terms, <$50K | Counsel | 企业微信/飞书/钉钉 |
| Counsel | Non-standard but within fallbacks, <$500K | GC | 企业微信/飞书/钉钉 or email |
...
Don't escalate things that are actually fine. If the term is within the fallbacks in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`, it doesn't need to go up.
...
Be specific. Not "escalate to legal leadership" — name the person or role from `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`. If the matrix doesn't name anyone for this situation, say so: "The escalation matrix doesn't cover [situation]. Suggest asking [GC name] who owns this."
...
```markdown
**Escalating to:** [name]
**Via:** [企业微信/飞书/钉钉 #channel / email / meeting — per `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`]
...
**What our playbook says:** [quote from `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`]
...
The calibration for what warrants escalation lives in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`, not in this skill. Check the playbook's stated position, its fallbacks, and its "automatic escalation regardless of dollar value" list:
...
If a term comes up that the playbook doesn't address, don't guess the threshold — ask the reviewing attorney whether this class of issue should escalate, and offer to record the answer in `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` so future reviews are consistent.
