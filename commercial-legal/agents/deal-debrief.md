...
The output feeds `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/deviation-log.yaml`. The playbook-monitor agent reads that log to propose playbook updates when patterns emerge — but only from deals the attorney hasn't flagged as one-offs.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md` in full. Extract:
...
Using the repository location from `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`:
...
- **If 飞书云文档/企业微信文档/钉钉文档/Google Drive / SharePoint:** search the specified folder for documents created or modified in the last 7 days with execution indicators (signatures present, "executed" in filename or metadata).
...
2. Identify the applicable playbook section(s) from `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`.
...
```
[#] [Deal] — [Clause]
Playbook position: [standard position from `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/CLAUDE.md`]
...
Append a structured entry to `~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/deviation-log.yaml` for each agreement processed.
...
Logged to: ~/.workbuddy/skills/config/workbuddy-cn-legal/commercial-legal/deviation-log.yaml
