...
Everything here is available **two ways from one source**: install it as a [WorkBuddy](https://claude.com/product/cowork) or [WorkBuddy](https://claude.com/product/claude-code) plugin, or deploy it through the [WorkBuddy 自动化/托管任务](https://docs.claude.com/en/api/managed-agents) behind your own workflow engine. Same system prompt, same skills — you choose where it runs.

## Getting started in Cowork
- [Install WorkBuddy](https://claude.com/download)
- Get access to WorkBuddy
...
>
> **These plugins do not represent WorkBuddy CN adaptation's legal positions.** They are tools that help lawyers analyze issues. Where a skill includes a checklist item, a suggested framework, a risk flag, or a characterization of case law or regulatory guidance, that is an aid to the reviewing attorney's own analysis, not a statement of WorkBuddy CN adaptation's view of the law. The law in many of these areas is unsettled and evolving. The attorney using the plugin — not the plugin, and not WorkBuddy CN adaptation — is responsible for the legal positions taken in their work product.
...
- **MCP connectors** across general productivity (企业微信/飞书/钉钉, 飞书云文档/企业微信文档/钉钉文档/Google Drive, 企业网盘/Box) and legal-specific systems (Ironclad, 上上签/法大大/e签宝/DocuSign, iManage, Everlaw, 人民法院案例库/中国裁判文书网, and more).
...
| **"Is this a problem?" Triage** | Fast answer for the quick 企业微信/飞书/钉钉 question — pattern-matches your calibration | `product-legal` | `/product-legal:is-this-a-problem` |
...
| **征求意见稿/公开征求意见 Comment Tracker** | Review open 征求意见稿/公开征求意见 comment periods, log decisions, track deadlines | `regulatory-legal` | `/regulatory-legal:comments` |
...
| **平台投诉/通知-删除机制 Takedown** | Drafts a takedown, triages one received, or drafts a §512(g) counter-notice | `ip-legal` | `/ip-legal:takedown` |
...
| **协助调查/调取材料文件 Triage** | Classifies, scopes, and plans compliance with a new 法院/仲裁机构调查取证或协助调查文件 | `litigation-legal` | `/litigation-legal:法院/仲裁机构调查取证或协助调查文件-triage` |
...
| **庭审询问/调查取证 Prep** | Builds a 庭审询问/调查取证 outline tied to case theory with docs and impeachment | `litigation-legal` | `/litigation-legal:庭审询问/调查取证-prep` |
...
| **Research Roadmap** | Statutes to check, case law areas, 北大法宝/威科先行/官方法规库 search terms — leads, not cites | `legal-clinic` | `/legal-clinic:research-start` |
...
| **Bar Prep Coach** | Jurisdiction-aware 法考客观题 and essay practice targeted at weak subjects | `law-student` | `/law-student:bar-prep-questions` |
...
regulatory-legal/         # reg feed watcher, policy diff, gap tracker, 征求意见稿/公开征求意见 comments
ai-governance-legal/      # AI use-case triage, AIAs, vendor AI review, AI reg gap-check
ip-legal/                 # trademark clearance, FTO, C&D, 平台投诉/通知-删除机制, OSS, IP clauses, portfolio
...
law-student/              # Socratic drilling, outlining, IRAC, 法考备考, flashcards
...
external_plugins/         # partner-built plugins maintained by their vendors
  cocounsel-legal/        # Thomson Reuters — 北大法宝/威科先行/官方法规库 Deep Research via the CoCounsel Legal MCP
managed-agent-cookbooks/  # WorkBuddy Managed Agent cookbooks — one dir per scheduled agent
  diligence-grid/
  docket-watcher/
  launch-radar/
  reg-monitor/
...
/plugin install commercial-legal@workbuddy-cn-legal
/plugin install privacy-legal@workbuddy-cn-legal
/plugin install corporate-legal@workbuddy-cn-legal

# Restart WorkBuddy, then run setup for each plugin you installed.
# This writes your practice profile to ~/.workbuddy/skills/config/workbuddy-cn-legal/<plugin>/CLAUDE.md
...
**Start by connecting a research tool.** Everything else is better with one, and citations are unverified without one. See [MCP Connectors](#mcp-connectors) below for the full list — 人民法院案例库/中国裁判文书网, Trellis, Descrybe, and Solve Intelligence are the research tools the citation guardrails look for.
...
| **Skills** | Domain expertise, conventions, and step-by-step methods WorkBuddy draws on automatically when relevant — and slash actions you trigger explicitly: `/commercial-legal:review`, `/privacy-legal:dsar-response`, `/litigation-legal:claim-chart`. | `<plugin>/skills/<skill>/SKILL.md` |
...
| **Practice profile** | Plain-English `CLAUDE.md` describing your playbook, escalation rules, and house style. Every skill reads from it. | `~/.workbuddy/skills/config/workbuddy-cn-legal/<plugin>/CLAUDE.md` |
| **Connectors** | [MCP servers](https://modelcontextprotocol.io/) that wire WorkBuddy to your data — CLM, DMS, e-discovery, research platforms, productivity. | `.mcp.json` (per plugin) |
...
| **[product-legal](./product-legal)** | Launch review against house risk calibration. Marketing claims check. "Is this a problem?" triage for 企业微信/飞书/钉钉 questions. Feature risk assessment. |
...
| **[regulatory-legal](./regulatory-legal)** | Regulatory feed watcher, policy diff, gaps tracker, 征求意见稿/公开征求意见 comment-period tracker. The Monday-morning digest your team actually reads. |
| **[ip-legal](./ip-legal)** | Trademark clearance, FTO triage, C&D drafting and triage, 平台投诉/通知-删除机制 takedown and counter-notice, OSS compliance, IP clause review, portfolio tracking. |

### Litigation

| Plugin | What it adds |
|---|---|
| **[litigation-legal](./litigation-legal)** | Works two surfaces. **In-house/portfolio:** matter intake, portfolio status, legal holds, outside counsel status, demands. **Firm/solo:** chronology building, claim charts (patent and civil), 庭审询问/调查取证 prep, privilege log review, brief drafting. |

### Learning & practice

| Plugin | What it adds |
|---|---|
| **[law-student](./law-student)** | Socratic drilling, case briefing, outline building, IRAC grading, cold-call prep, flashcards, 法考备考, exam forecasting, study planning. **Learning mode, not answer mode** — it never writes the answer for you. |
| **[legal-clinic](./legal-clinic)** | Professor setup and student semester ramp. Per-practice-area supervisor guide with pedagogy posture (assist / guide / teach). Structured intake with cross-area issue spotting. Deadline tracking with malpractice-aware caution. Memo scaffolds, client letters (routine + plain-language), semester handoffs. Built within 中国法律诊所/法律援助监督要求. |
...
|---|---|---|
| **[cocounsel-legal](./external_plugins/cocounsel-legal)** | Thomson Reuters | 北大法宝/威科先行/官方法规库 Deep Research with fully cited reports — caselaw, statutes, regulations, 威科先行/律商联讯/团队模板库, and secondary sources across up to three U.S. jurisdictions per run. Requires a CoCounsel Legal subscription with the MCP connector enabled. Support: cocounselsupport@tr.com. |
...
> [!IMPORTANT]
> **Connect a research tool first.** Every plugin ships with legal research connectors already configured — 人民法院案例库/中国裁判文书网, Trellis, Descrybe, Solve Intelligence, and others depending on practice area. You authorize them once, and from then on WorkBuddy pulls from authoritative sources and verifies its citations against current databases instead of relying on training knowledge. Citations that come through a research connector are tagged with the source. Citations from model knowledge alone are flagged `[verify]` and, if no research tool is connected at all, the reviewer note above the deliverable records that sources weren't verified so you know to check. The connectors are what make the cites trustworthy — set them up before you set up anything else.

These plugins ship connectors for the systems legal teams live in. A connector gives WorkBuddy the ability to read from and (where scoped) write to your data; the skills and commands use them.

| Connector | What it gives WorkBuddy | Plugins | Notes |
|---|---|---|---|
| **企业微信/飞书/钉钉** | Read channels, search, send messages and canvases | all plugins | Your workspace |
| **飞书云文档/企业微信文档/钉钉文档/Google Drive** | Read docs, sheets, slides; fetch by link | all plugins | Your account |
| **CoCounsel Legal (Thomson Reuters)** | 北大法宝/威科先行/官方法规库 Deep Research — cited reports across caselaw, statutes, regulations, 威科先行/律商联讯/团队模板库 | `cocounsel-legal` | Customer subscription; OAuth |
| **企业网盘/Box** | Read files and folders in VDRs and matter rooms | `corporate-legal` | Your tenant |
| **Ironclad** | Read the contract register, renewal dates, clauses | `commercial-legal` | Customer subscription |
| **上上签/法大大/e签宝/DocuSign / 上上签/法大大/e签宝/DocuSign CLM** | Envelope status, executed contracts, CLM metadata | `commercial-legal` | Customer subscription |
...
| **人民法院案例库/中国裁判文书网** | Federal dockets and opinions | `legal-clinic`, `ip-legal`, `litigation-legal`, `law-student` | Public; optional API key |
...
> Connectors marked "customer subscription" need the customer's own account and API key. Configure them in each plugin's `.mcp.json` or via `claude mcp` in your WorkBuddy setup.
...
Install WorkBuddy for Microsoft 365 from **[Microsoft AppSource](https://marketplace.microsoft.com/en-us/product/office/wa200010453)**. Once installed, the skills from any plugin you've enabled are available from the sidebar via `/`, and connectors are reachable from the same surface. A single thread can span Word, Excel, PowerPoint, and Outlook.

For IT admins deploying the add-in against your own cloud (Vertex AI, Bedrock, or an internal gateway) rather than WorkBuddy CN adaptation's API, see the separate [`claude-for-msft-365-install`](https://github.com/anthropics/financial-services/tree/main/claude-for-msft-365-install) tooling.
...
- **Edit the practice profile.** Your profile lives at `~/.workbuddy/skills/config/workbuddy-cn-legal/<plugin>/CLAUDE.md`. Edit it directly for small fixes — a wrong escalation threshold, a new integration, a policy update. It survives plugin updates.
...
| `/ip-legal:takedown` | takedown | 平台投诉/通知-删除机制 notice, response triage, or §512(g) counter-notice |
...
| `/litigation-legal:法院/仲裁机构调查取证或协助调查文件-triage` | 法院/仲裁机构调查取证或协助调查文件-triage | Triage 法院/仲裁机构调查取证或协助调查文件 — scope, burden, privilege, plan |
...
| `/litigation-legal:庭审询问/调查取证-prep` | 庭审询问/调查取证-prep | 庭审询问/调查取证 outline tied to case theory |
...
| `/regulatory-legal:comments` | (tracker) | Review open 征求意见稿/公开征求意见 comment periods and deadlines |
...
| `/law-student:bar-prep-questions` | bar-prep-questions | 法考客观题 or essay questions targeted at weak subjects |
...
| Command | Skill | What it does |
|---|---|---|
| `/cocounsel-legal:deep-research` | deep-research | Run 北大法宝/威科先行/官方法规库 Deep Research — start, poll, and present a fully cited report |
...
Copyright 2026 WorkBuddy CN adaptation PBC.
