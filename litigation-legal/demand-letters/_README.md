...
After a demand letter is drafted, `demand-draft` assesses materiality (heuristic from house `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`) and offers to create a matter. If yes, a matter row goes into `matters/_log.yaml` with `source: demand-letter`, and `matters/[matter-slug]/matter.md` links back to this demand-letter's folder.
