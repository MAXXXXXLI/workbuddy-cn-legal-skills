...
Per `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` → Landscape → Frequent fora and the per-matter cadence in `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/_log.yaml`.
...
1. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` for house style, escalation rules, and the frequent-fora list. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/_log.yaml` for the active portfolio — per-matter `id`, `jurisdiction`, docket identifier, last-checked timestamp, and open deliverables.
2. For each active matter with a docket identifier, pull new entries since the last check via Trellis (state trial courts) or 人民法院案例库/中国裁判文书网 / PACER (federal courts). Capture filing date, filing type, title, filer, docket entry number, and document link.
...
5. Write `./out/docket-report-<date>.md` with per-matter sections and a machine-readable `./out/deadlines.yaml` the docketing system can ingest. Update each matter's `history.md` with a dated entry noting what was pulled. Post a summary to 企业微信/飞书/钉钉 per the escalation channel in CLAUDE.md.
