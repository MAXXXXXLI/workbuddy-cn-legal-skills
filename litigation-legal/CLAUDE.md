...
4. On first run after a plugin update, if a populated CLAUDE.md exists at the old cache path
   (~/.workbuddy/skills/cache/workbuddy-cn-legal/litigation-legal/<version>/CLAUDE.md for any version)
...
本配置模板已从 Claude 法律插件转换为 WorkBuddy 中国语境版本。模块：**争议解决法务**。
...
**Org / legal entity:** [PLACEHOLDER — e.g., "Acme Corporation, a 公司注册地/章程约定地（中国公司须按《公司法》和章程核验） corporation"] *(From company-profile.md — edit there to change across all plugins)*
...
*Skills that branch on side: `demand-draft` / `demand-received`, `法院/仲裁机构调查取证或协助调查文件-triage`, `matter-intake` (per-matter), `chronology` (offensive vs defensive framing), `claim-chart` (proving vs disproving elements).*
...
| Document storage (Google 飞书云文档/企业微信文档/钉钉文档/SharePoint) | [✓ / ✗] | Manual file paths; matter folders local only |
...
- If Role in `## Who's using this` is Lawyer / legal professional: `保密 / 内部法律分析 — 律师工作底稿/内部法律分析 — PREPARED AT THE DIRECTION OF COUNSEL`
- If Role is Non-lawyer: `RESEARCH NOTES — NOT LEGAL ADVICE — REVIEW WITH A 执业律师/法务负责人 BEFORE ACTING`
...
- `[北大法宝/威科先行/官方法规库]` / `[人民法院案例库/中国裁判文书网]` / `[Trellis]` / `[Descrybe]` — ONLY if the citation appears in a tool result from that MCP in this conversation.
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/verification-log.md`:
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` (and `~/.workbuddy/skills/config/workbuddy-cn-legal/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
...
| 协助调查/调取材料文件s (third-party) | [PLACEHOLDER] | | |
...
**Frequent fora:** [PLACEHOLDER — e.g., 公司注册地/章程约定地（中国公司须按《公司法》和章程核验） Chancery, N.D. Cal., S.D.N.Y., AAA / JAMS arbitration]
...
|---|---|---|---|
| [PLACEHOLDER e.g. "飞书云文档/企业微信文档/钉钉文档/Google Drive — Legal"] | cloud drive | [path / root folder] | [yes/no] |
...
**Default matter folder pattern:** [PLACEHOLDER — e.g., "G:/Legal/Matters/{matter-slug}" or "企业网盘/Box → Legal → Matters → {matter-name}"]
...
**Channel:** [PLACEHOLDER — e.g., "GC: email + 企业微信/飞书/钉钉 DM for urgent; CFO: email only; board: via GC"]
