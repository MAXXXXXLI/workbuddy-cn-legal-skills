...
description: >
  ...
  [name]'s 庭审询问/调查取证". WorkBuddy
  中国语境适配：默认中国大陆法域，用于争议解决法务场景下的庭审/询问准备。中文触发词包括：中国法、中国合规、争议解决法务、庭审/询问准备、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**争议解决法务**；当前技能：**庭审/询问准备**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` → case theory, key facts.
...
For US 庭审询问/调查取证s, declarations, and affidavits: different rules, but the same discipline applies. A declaration in the declarant's voice that the declarant didn't write is a credibility problem at best.
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` → case theory (theory, pivot fact, key facts for/against), eDiscovery platform.

**Conflicts gate — unbypassable.** Before building an outline, check `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:

> "I don't see [matter slug] in the matter log. Run `/litigation-legal:matter-intake` first so the conflicts check runs and the matter workspace is set up. I won't build a 庭审询问/调查取证 outline on a matter that hasn't been intaken — the conflicts check is the gate."
...
- Facts from `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` → key facts for us, that this witness can establish
...
# 庭审询问/调查取证 Outline: [Witness Name]
...
**Applicable 庭审询问/调查取证 rules:** [FRCP 30 / state rule / local rule / standing order — with pinpoint cites] `[UNCERTAIN — verify currency]`
...
**Cite check any authority relied on.** Rule citations (FRCP 30, state equivalents, local rules, standing orders) and any case law pulled into the outline were generated by an AI model. Verify each against 北大法宝/威科先行/官方法规库, 人民法院案例库/中国裁判文书网, or your research platform — confirm currency and scope before using at the 庭审询问/调查取证. Source tags on each citation (e.g., `[北大法宝/威科先行/官方法规库]`, `[web search — verify]`) show where the cite came from; `verify` tags carry higher fabrication risk and should be checked first.
```

## What this skill does not do

- Take the 庭审询问/调查取证. The outline is a map; the attorney drives.
