...
name: "争议解决法务-事项关闭"
description: >
  用于中国大陆争议解决法务场景下的事项关闭。输出默认简体中文；正式依赖前需法务负责人或执业律师核验。
  Close a matter — capture outcome, final exposure, and lessons, then archive it out of
  the active portfolio without deleting the record. Use when the user wants to close a
  matter, says "[matter] is done", or needs to record a settlement, dismissal, judgment,
  withdrawal, or consolidation outcome. WorkBuddy
  中国语境适配：默认中国大陆法域，用于争议解决法务场景下的事项关闭。中文触发词包括：中国法、中国合规、争议解决法务、事项关闭、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**争议解决法务**；当前技能：**事项关闭**。
...
5. Append final entry to `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/history.md`.
6. Matter stays in `_log.yaml` and `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/` — not deleted. `/portfolio-status` filters it from active rollups.
...
- `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/matter.md` — reference (intake context)
...
**Before closing the matter (the consequential act — the matter is archived and active tracking ends):** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
### Append final entry to `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/history.md`
...
### Touch `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/matter.md`
