...
description: >
  ...
  needs a first draft of a brief section. WorkBuddy
  中国语境适配：默认中国大陆法域，用于争议解决法务场景下的诉讼文书章节起草。中文触发词包括：中国法、中国合规、争议解决法务、诉讼文书章节起草、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**争议解决法务**；当前技能：**诉讼文书章节起草**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` → case theory, house style.
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` → case theory, house style (citation format, structure, tone, length norms).

**Conflicts gate — unbypassable.** Before drafting, check `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/_log.yaml` for the matter slug this skill is being invoked on. If the matter is not in `_log.yaml`, refuse and route:
...
Per `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`:
...
**Before the brief is filed (the consequential act — this skill drafts, but the gate runs at the filing step regardless of who triggers it):** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Cite check before filing.** Citations in this draft were generated by an AI model and have not been verified against a primary source. Run every case, statute, and regulation through 北大法宝/威科先行/官方法规库, 人民法院案例库/中国裁判文书网, or your firm's research platform for accuracy, good-law status, and subsequent history. Fabricated or misquoted citations in filed briefs have resulted in Rule 11 sanctions.

**Draft only — not a filing.** Filing this section initiates (or participates in) a proceeding and carries Rule 11 / Rule 3.3 exposure. A 执业律师/法务负责人 reviews, edits, and takes professional responsibility before it goes on the docket. Do not file unreviewed.
```
