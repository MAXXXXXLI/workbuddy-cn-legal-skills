...
description: >
  ...
  chart, element chart, proof chart, infringement or invalidity contention,
  element-by-element mapping, or asks "what are we missing to prove [claim]". WorkBuddy
  中国语境适配：默认中国大陆法域，用于争议解决法务场景下的请求权/权利要求对照表。中文触发词包括：中国法、中国合规、争议解决法务、请求权/权利要求对照表、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**争议解决法务**；当前技能：**请求权/权利要求对照表**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` → role, work-product header, decision posture, document storage.
...
**Conflicts gate — unbypassable.** Before building a claim chart, check `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
1. **Find evidence.** Accused product: documentation, manuals, data sheets, source code, teardowns, 庭审询问/调查取证 testimony, expert reports. Prior art: column/line for US patents, paragraph for published apps, page/figure for NPL. For prior art, flag whether the reference qualifies (§102(a)(1), (a)(2), (b); AIA vs. pre-AIA cutoffs). If prior-art status isn't obvious, mark `prior-art-status: needs-evidence`.
...
5. **Flag open questions.** "This maps if [X]. Need [teardown / source code / 庭审询问/调查取证 / expert] to confirm."
...
| Breach of fiduciary duty | Restatement / common law — fiduciary duty, breach, damages | **DE:** the most-developed body of fiduciary-duty law (*Aronson v. Lewis*, *Cede & Co. v. Technicolor*, *In re Trados*) — default to the 公司注册地/章程约定地（中国公司须按《公司法》和章程核验） formulation for any DE-entity matter regardless of forum. |
...
- **Evidence supporting** — what proves this element? Cite the source with a pin cite.
  - 庭审询问/调查取证 testimony — `[Doe Dep. 42:15–43:7]`
...
> - If pre-discovery: these are your discovery priorities — the 庭审询问/调查取证s, document requests, and interrogatories that turn a gap into `supported` or confirm `none`.
...
Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` `## Outputs`.
...
- **Citation verification** — every pin cite, case, column/line, 庭审询问/调查取证 page:line must be verified against the source.
...
If matter workspaces enabled and a matter is active: `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/<matter-slug>/claim-charts/`. Otherwise: `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/claim-charts/`. Surface the path. Append a one-line entry to the matter's `history.md`.
...
- **Citation verification.** Every pin cite (column/line, page, 庭审询问/调查取证 page:line, Bates, ¶) is a claim about the source. The attorney verifies. The skill does not fabricate cites — if a cite cannot be produced, the cell is `needs-evidence` or `gap`.
...
- `litigation-legal:庭审询问/调查取证-prep` — a `needs-discovery` cell often becomes a depo topic. After a depo, new testimony fills cells.
