...
description: >
  ...
  from the production" or "what happened when", or needs a working, statement-of-facts, or
  witness-specific timeline. WorkBuddy
  中国语境适配：默认中国大陆法域，用于争议解决法务场景下的事实时间线。中文触发词包括：中国法、中国合规、争议解决法务、事实时间线、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**争议解决法务**；当前技能：**事实时间线**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/matter.md` → theory, pivot fact, key facts.
2. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` → Document storage sources, default matter folder pattern.
3. Follow the workflow and reference below.
4. Identify sources in order: user-provided paths this session, default matter folder, declared sources from `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`.
...
8. Write `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/chronology.md` (or format variant per flag).
...
- **`--matter` mode (default for in-house litigation counsel).** Matter-history-focused. Reads the matter's case theory and key facts from `matter.md`, pulls from declared document-storage sources (飞书云文档/企业微信文档/钉钉文档/Google Drive, SharePoint, Gmail, iManage, CLM — whatever the `## Landscape` section of CLAUDE.md declares), and treats `history.md` as the running internal log (decisions, holds, reserve memos — intentionally not in the chronology). Output is matter-centric: what happened across the dispute, tagged for advocacy use.
...
- `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/matter.md` → case theory, key facts, pivot fact (for significance tagging), key dates.
...
**Conflicts gate — unbypassable (`--matter` mode).** Before building the chronology, check `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
Location: `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/chronology.md`. Complete, tagged, annotated. The reference doc counsel works from.
