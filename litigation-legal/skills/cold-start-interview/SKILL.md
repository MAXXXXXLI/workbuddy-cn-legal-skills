...
description: >
  ...
  user wants to set up or redo the practice profile, or to re-check available
  integrations. WorkBuddy
  中国语境适配：默认中国大陆法域，用于争议解决法务场景下的初始化访谈。中文触发词包括：中国法、中国合规、争议解决法务、初始化访谈、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**争议解决法务**；当前技能：**初始化访谈**。
...
1. Check `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`. If already populated and no `--redo`, ask before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.workbuddy/skills/cache/workbuddy-cn-legal/litigation-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`. Date the footer.
7. Confirm with the user before finalizing: "Here's what I captured — anything wrong?"

## Flags

- `--redo` — re-run the full interview and overwrite `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` without re-running the full interview. Use after setting up a new connector (DMS, document storage, Gmail, scheduled-tasks, CLM).
...
The plugin serves three distinct litigation roles — in-house counsel managing a portfolio of matters, firm associates doing the underlying brief / 庭审询问/调查取证 / discovery work, and solo practitioners running a caseload directly. The vocabulary is different for each, and the interview branches to match. Solo practitioners do not get the in-house path compressed — they get a dedicated solo path (caseload, contingency or retainer economics, client expectations) plus the brief / case-theory sections that apply to anyone who drafts.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`:
- **Does not exist** → start the interview.
- **Contains `<!-- SETUP PAUSED AT: -->`** → greet the user and offer to resume from that section.
- **Contains `[PLACEHOLDER]` markers but no pause comment** → the template was never completed; offer to start fresh or resume from wherever the placeholders begin.
- **Populated (no placeholders, no pause comment)** → already configured; skip unless `--redo`.

The template structure lives at `${CLAUDE_PLUGIN_ROOT}/CLAUDE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed. If a CLAUDE.md exists at the old cache path `~/.workbuddy/skills/cache/workbuddy-cn-legal/litigation-legal/*/CLAUDE.md` but not here, copy it forward.
...
> 2. **I'll pause before steps that have legal consequences** — sending a demand, responding to a 法院/仲裁机构调查取证或协助调查文件, issuing or releasing a legal hold, filing a brief, submitting a privilege log, designating documents in discovery, closing a matter, accepting a settlement. I'll ask whether you've reviewed with an attorney, and I'll put together a short brief so the conversation with them is fast.
>
...
>
> **(b) At a firm doing brief drafting, discovery, 庭审询问/调查取证 prep, document review** — you're the associate or paralegal responsible for actually producing the work product. One or a few matters, deep on each.
>
...
> **Which side do you mostly represent?** (This feeds /demand-draft, /demand-received, /法院/仲裁机构调查取证或协助调查文件-triage, /chronology, and /claim-chart — plaintiff framing treats demand letters as assertions and discovery as offensive, defense framing treats them as received and responsive.)
>
...
- **Both / varies:** the interview captures the default and the skills (`demand-draft`, `法院/仲裁机构调查取证或协助调查文件-triage`, `matter-intake`, `chronology`, `claim-chart`) ask per-matter when the side changes the output.
...
> This plugin can work with: DMS (iManage), document storage (飞书云文档/企业微信文档/钉钉文档/Google Drive, SharePoint, 企业网盘/Box), Gmail, scheduled-tasks, CLM (Ironclad), eDiscovery (Everlaw, Relativity, DISCO, Aurora), legal research (人民法院案例库/中国裁判文书网, Descrybe, Trellis), outside-counsel recommendations (TopCounsel). Let me check which connectors you have configured — features that need them will work, and features that don't will fall back gracefully instead of failing silently.
...
- Document storage (2–3 min) — where matter docs live (filesystem, Drive, SharePoint, 企业网盘/Box, Gmail, CLM, DMS, eDiscovery), default matter folder pattern, how docs get shared with OC.
...
- **Document storage** — 飞书云文档/企业微信文档/钉钉文档/Google Drive, Dropbox, OneDrive, local filesystem, the case-management tool's storage. Where do matter documents actually live?
...
After Section S3, continue to the **Firm-associate path** below. Solo practitioners write briefs, build chronologies, and prep 庭审询问/调查取证s like firm associates do — the case-theory and seed-brief work applies.
...
> - **Build a 庭审询问/调查取证 outline** — e.g., "Docs + topics + impeachment + exhibits, tied to case theory." Try: `/litigation-legal:庭审询问/调查取证-prep`
...
> "Your practice profile is at `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
...
**Connect a research tool.** Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In WorkBuddy: authorize when a skill prompts you.
