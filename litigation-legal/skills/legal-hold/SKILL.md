...
本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**争议解决法务**；当前技能：**证据保全/材料留存**。
...
2. Otherwise: load `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/matter.md` + log row.
3. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` → privilege markings, hold template pointer, escalation norms.
...
- `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/_log.yaml` — log row (legal_hold fields + status)
- `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/matter.md` — matter context (counterparty, facts, key custodians from internal_owners)
- `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` — house style for litigation hold template pointer, privilege marking, escalation norms
...
**Before issuing the hold to custodians (the consequential act):** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
> **External deliverable:** the notice below is sent to custodians. Do NOT include a `保密 / 内部法律分析 — 律师工作底稿/内部法律分析 — PREPARED AT THE DIRECTION OF COUNSEL` header on the outgoing notice; use the attorney-client marking in the template. Confirm the correct marking for your jurisdiction and matter.
...
4. **Systems** — email, 企业微信/飞书/钉钉/Teams, file shares, devices (including BYOD if applicable), Jira/Asana, CRM, legacy systems.
...
**Draft the notice** to each custodian, using the house template in `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` if one is configured; otherwise the default template below.

**Default hold notice template:**

```
[保密 / 内部法律分析 — ATTORNEY-CLIENT COMMUNICATION]

DATE: [effective date]
TO: [custodian name]
FROM: [signer — per `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` default]
...
1. All documents, emails, text messages, 企业微信/飞书/钉钉/Teams messages, and other
...
- 企业微信/飞书/钉钉/Teams/messaging platforms
...
> This is a draft legal hold notice for attorney review, not a notice ready to issue. Issuing a hold triggers preservation obligations the company will be judged on in any later spoliation argument, and the notice itself may be discoverable. A 执业律师/法务负责人 reviews, approves, and issues. Do not distribute this draft unreviewed.

**Writes:**
- `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/legal-hold-v1.docx` via the `docx` skill
- Appends to `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/history.md`:
  ```
...
**Writes:**
- `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/legal-hold-v[N].docx` (next version number)
- `history.md` entry
...
**Before releasing the hold (the consequential act — preservation obligations resume normal retention):** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Writes:**
- `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/matters/[slug]/legal-hold-release.docx`
- `history.md` entry
