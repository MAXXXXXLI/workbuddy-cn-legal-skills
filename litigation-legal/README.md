...
Your configuration is stored at `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` and survives plugin updates.

## Commands

| Command | Does |
|---|---|
| `/litigation-legal:cold-start-interview` | Cold-start → writes house `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` |
...
| `/litigation-legal:法院/仲裁机构调查取证或协助调查文件-triage [path]` | Triage a 法院/仲裁机构调查取证或协助调查文件 — classify, scope/burden/privilege, objections framework, compliance plan |
...
| **法院/仲裁机构调查取证或协助调查文件-triage** | Classify 法院/仲裁机构调查取证或协助调查文件, analyze scope/burden/privilege, produce objections framework + compliance plan |
...
│       ├── draft-v1.docx
│       └── checklist.md
├── inbound/                           # incoming demands, 法院/仲裁机构调查取证或协助调查文件s, regulator letters
│   └── [slug]/
│       ├── incoming.[ext]
│       ├── triage.md
...
Your practice profile at `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. You can re-run setup, edit the file directly, or tell a skill to record a new position.

## Notes

- Every skill reads from `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` first. If your risk appetite changes or you bring on new outside counsel, update it — don't paper over it in individual matters.
- `## Company profile` is the first section of `~/.workbuddy/skills/config/workbuddy-cn-legal/litigation-legal/CLAUDE.md` by convention. If you run other `-legal` plugins, you can copy it across rather than re-entering the same context.
...
- `[SME VERIFY: specific judgment call]` — a judgment (merit read, significance tag, objection strength, privilege status) that requires subject-matter expert review. SME = 执业律师/法务负责人 qualified in the relevant jurisdiction / area. Used liberally — anything judgment-heavy should carry this.
