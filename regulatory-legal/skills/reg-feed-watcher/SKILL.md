...
description: >
  ...
  regulatory development for classification and diff. WorkBuddy
  中国语境适配：默认中国大陆法域，用于监管合规法务场景下的监管动态监测。中文触发词包括：中国法、中国合规、监管合规法务、监管动态监测、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**监管合规法务**；当前技能：**监管动态监测**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold, feed config.
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold, feed configuration, digest output path (if set).
...
> **Coverage gap noticed:** Your watchlist includes [category], but only [N] feeds are configured. The source catalog lists [X] options in this category (e.g., [top 2-3 names]). Want me to suggest additions? Run `/regulatory-legal:cold-start-interview --redo` to update, or edit `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` directly.
...
- **Federal Register API** (`https://www.federalregister.gov/api/v1/documents`)
  ...
  deadline (for 征求意见稿/公开征求意见s), and citation. Covers all US federal agencies.
- **Direct regulator RSS** — fetch and parse any RSS URLs in ~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md feed
...
Each item gets a materiality tier per `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`:

| Item type | Match against threshold |
|---|---|
| Final rule | Usually "always material" |
| Proposed rule / 征求意见稿/公开征求意见 | Usually "review-worthy" — and always log comment deadline |
...
**ANPR / RFI handling — specific.** Pre-rule items are distinct from 征求意见稿/公开征求意见s in one important way: they don't change the law, but they do carry comment deadlines and they signal the regulator's direction. Treat them as a separate branch:
...
- **Do** log the comment deadline to `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/comment-tracker.yaml` with `item_type: ANPR` or `item_type: RFI` so the downstream tracker can distinguish these from compliance gaps.
...
**征求意见稿/公开征求意见 comment deadline handling:**

For every 征求意见稿/公开征求意见 classified at any tier above "skip":
- Extract comment deadline (Federal Register API returns this as structured data)
- If comment tracking is enabled in ~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md: append to `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/comment-tracker.yaml`
  with status "undecided" and the default comment decision owner from ~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md
...
1. Look for `Digest output path` in `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`. If set, write there. Default if unset: `~/regulatory-legal-digests/reg-digest-YYYY-MM-DD.md`.
...
[征求意见稿/公开征求意见s: include "💬 Comment deadline: [date] — decision pending" if comment tracking enabled]
...
**Comment tracker:** [N] 征求意见稿/公开征求意见s with open comment decisions — run /regulatory-legal:comments to review

---

**Verify citations before relying on them.** Regulatory citations here were AI-generated and have not been checked against a primary source. Before acting on any rule, guidance, or enforcement action above, confirm it against 北大法宝/威科先行/官方法规库, your firm's research platform, or the issuing authority's website — check accuracy, effective date, and current status. AI-generated regulatory citations are sometimes fabricated, misquoted, or stale. Source tags on each item (e.g., `[Federal Register]`, `[web search — verify]`) show where the citation came from; `verify` tags carry higher fabrication risk and should be checked first.
```

## Config-dependent fallbacks

This skill reads the watchlist, materiality threshold, and feed configuration from `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`. When a required value is still `[PLACEHOLDER]` or empty, say so in the output — specifically, not generically:

- **Watchlist empty:** stop and say "The watchlist in your configuration is empty. I can't pull feeds without knowing which regulators to watch. Run `/regulatory-legal:cold-start-interview --redo` or edit `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` and add at least one regulator."
- **Materiality threshold empty:** fall back to the default tiers and append: "This output used the default materiality tiers because your configuration doesn't have custom thresholds set. Tune them with `/regulatory-legal:cold-start-interview --redo` or by editing `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`."
- **Feed configuration empty:** run Federal Register API only and append: "This output used only the free Federal Register API because your configuration doesn't list direct RSS or paid feeds. Add feeds with `/regulatory-legal:cold-start-interview --redo` or by editing `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`."
...
- **To comment-tracker:** Any 征求意见稿/公开征求意见 classified above "skip" → comment deadline logged automatically if tracking is enabled.
...
- Change the materiality threshold. If the filter is wrong, edit ~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md.
- Require TR or 人民法院案例库/中国裁判文书网. Free feeds are the baseline; paid feeds add depth.
