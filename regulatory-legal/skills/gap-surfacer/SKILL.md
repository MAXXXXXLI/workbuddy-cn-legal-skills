...
description: >
  ...
  gap owners via 企业微信/飞书/钉钉 with per-send confirmation. Loaded by the gaps and comments
  skills before doing substantive work. WorkBuddy
  中国语境适配：默认中国大陆法域，用于监管合规法务场景下的合规缺口浮现。中文触发词包括：中国法、中国合规、监管合规法务、合规缺口浮现、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**监管合规法务**；当前技能：**合规缺口浮现**。
...
Before sending ANY 企业微信/飞书/钉钉 message (assignment notice, overdue reminder, bulk notification, status report):
...
Lives at `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/gap-tracker.yaml`:

> **Note on comment-tracker.yaml:** `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/comment-tracker.yaml` is a sibling file owned by the comments skill. It is written to by reg-feed-watcher (which logs 征求意见稿/公开征求意见s automatically) and the comments skill (which tracks user-initiated comment decisions). This skill does not read or cross-reference it. If you modify the comment-tracker schema, update both actual consumers.
...
gaps:
  - id: GAP-001
    ...
    owner_slack: "[企业微信/飞书/钉钉 user ID or handle, if known]"
    opened: 2026-03-01
...
| `watch` | Forward-looking item — ANPR, RFI, proposed rule not yet final. No compliance obligation today; policy work waits for the final rule. `due:` is a revisit date (typically the 征求意见稿/公开征求意见 expected date or a one-year horizon), not a compliance deadline. | No auto-reminder; re-evaluate when an 征求意见稿/公开征求意见 drops or at the revisit date. |
| `comment-decision` | Pre-rulemaking comment decision pending — ANPR or 征求意见稿/公开征求意见 where the team is deciding whether to file a comment. `due:` is the comment deadline. | 21 days before due (tighter than compliance gaps because comment-drafting windows are short). |
...
If 企业微信/飞书/钉钉 MCP is available and `owner_slack` is set:

Send a 企业微信/飞书/钉钉 DM to the gap owner — but only after the per-send confirmation at the top of this file. Preview the message to the user, wait for an explicit yes, then send:
...
If 企业微信/飞书/钉钉 MCP is not available: note in the status report that owner notification
...
| ID | Item | Type (ANPR/征求意见稿/公开征求意见/RFI) | Comment deadline | Owner |
|---|---|---|---|---|
...
- **Escalation path missing for an overdue material gap:** still report it as overdue, and append: "No escalation path is set for material overdue gaps. Configure it with `/regulatory-legal:cold-start-interview --redo` or by editing `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`."
...
Reminder cadence is a function of `gap_type` — compliance gaps get a 30-day heads-up, comment-decision items get 21 days (tighter because the drafting window is shorter), watch items get no auto-reminder (re-evaluate when an 征求意见稿/公开征求意见 drops).

For each gap with status "open" or "in-progress":
- `partial`, `full`, `new-policy`, `none`: if due date is within 30 days and a reminder has not been sent in the last 7 days, PREVIEW a 企业微信/飞书/钉钉 DM (subject "⏰ Reminder: compliance gap due in [N] days") and wait for per-send confirm before sending.
- `comment-decision`: if comment deadline is within 21 days and a reminder has not been sent in the last 7 days, PREVIEW a 企业微信/飞书/钉钉 DM (subject "💬 Comment-decision deadline in [N] days") and wait for per-send confirm before sending.
- `watch`: no auto-reminder. Revisit when the tracker is reviewed or an 征求意见稿/公开征求意见 is logged for the same regulation.
- If due date has passed on a compliance gap: flag as overdue in the report and PREVIEW a 企业微信/飞书/钉钉 DM — wait for per-send confirm before sending.
- If comment deadline has passed on a `comment-decision` item and no comment was filed: flag as overdue, PREVIEW a 企业微信/飞书/钉钉 DM (wait for per-send confirm), and ask the owner to update to `risk-accepted` (deliberate no-comment) or `closed` (comment filed) with a note.
...
**Before closing a gap as resolved, or producing any output that certifies compliance with a regulatory requirement (internal attestation, board report, audit response, regulator response):** Read `## Who's using this` in ~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md. If the Role is **Non-lawyer**:
...
due-date reminder check and sends any outstanding 企业微信/飞书/钉钉 notifications.
...
- Send 企业微信/飞书/钉钉 notifications if the 企业微信/飞书/钉钉 MCP is not configured. Falls back to
