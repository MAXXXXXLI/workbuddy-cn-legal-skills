...
description: >
  ...
  "redraft the policy", "draft the policy fix", "mark up the policy", or when gap-surfacer
  hands off a gap for drafting. WorkBuddy
  中国语境适配：默认中国大陆法域，用于监管合规法务场景下的政策修订草案。中文触发词包括：中国法、中国合规、监管合规法务、政策修订草案、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**监管合规法务**；当前技能：**政策修订草案**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` → policy library index + practice profile.
...
One of:
- A `GAP-ID` from the gap tracker — load the entry from `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/gap-tracker.yaml` (or the matter-level equivalent).
...
This skill reads the policy library index and owners from `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`. When a value it needs is empty or still `[PLACEHOLDER]`:
