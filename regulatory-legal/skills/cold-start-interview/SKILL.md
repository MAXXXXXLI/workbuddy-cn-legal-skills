...
description: >
  ...
  responding (--check-integrations). WorkBuddy
  中国语境适配：默认中国大陆法域，用于监管合规法务场景下的初始化访谈。中文触发词包括：中国法、中国合规、监管合规法务、初始化访谈、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**监管合规法务**；当前技能：**初始化访谈**。
...
1. Check `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`. If a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.workbuddy/skills/cache/workbuddy-cn-legal/regulatory-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and tell the user what was migrated. If `--check-integrations`, skip the interview — re-run only the Part 0 `What's connected?` check and rewrite the `## Available integrations` table in `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`.
...
4. Write `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` (creating parent directories as needed) with watchlist + materiality threshold.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `~/.workbuddy/skills/cache/workbuddy-cn-legal/regulatory-legal/*/CLAUDE.md` but not at the config path, copy it forward.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/regulatory-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /regulatory-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> Who'll be using this plugin day to day? (This feeds every skill's work-product header and output framing — lawyer gets "律师工作底稿/内部法律分析," non-lawyer gets research framing and attorney-review checkpoints before regulator-facing steps.)
>
...
> This plugin can work with: regulatory feeds (Thomson Reuters Regulatory Intelligence), document storage (飞书云文档/企业微信文档/钉钉文档/Google Drive, SharePoint, 企业网盘/Box), and 企业微信/飞书/钉钉. Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "TR Regulatory Intelligence isn't connected. In WorkBuddy: Settings → Connectors → Add → Thomson Reuters → sign in. In WorkBuddy: add the TR MCP to your config or via `/mcp`. This plugin works without it — Federal Register + manual paste covers US federal coverage — but connecting it adds enrichment and alert import."
...
- A proposed rule (征求意见稿/公开征求意见) — comment period open
...
- 人民法院案例库/中国裁判文书网? Which trackers?
...
> When I see a proposed rule (征求意见稿/公开征求意见) from your watchlist, I'll automatically log the comment deadline. Do you want me to flag these so you can decide whether to file a comment?
...
| 人民法院案例库/中国裁判文书网 | [yes/no] | [tracker names] |
...
> - **Track 征求意见稿/公开征求意见 comment periods** — e.g., "What's open, comment deadlines, and a decision log on whether to file." Try: `/regulatory-legal:comments`
>
...
- Close with the changeability note:
  ...
  > "Done. Your configuration is at `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` — a plain-text file you can read and edit directly. Anything you answered can be changed:
  >
