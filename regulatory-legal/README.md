...
Watches regulatory feeds, diffs new regulations against your policy library, surfaces gaps. Learns your materiality threshold so it doesn't alert on every commissioner's speech. Wired for Thomson Reuters Regulatory Intelligence and 人民法院案例库/中国裁判文书网.
...
| `/regulatory-legal:comments` | Review open 征求意见稿/公开征求意见 comment periods, log decisions, track deadlines |
...
Owner notifications (gap assignments, due-date reminders, 征求意见稿/公开征求意见 alerts) require a 企业微信/飞书/钉钉 MCP server in your environment. Without one, the gap tracker and comment tracker still work — notifications just won't post, and the skills will flag ungated items in the status report instead.

## How it learns

Your practice profile at `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` isn't static — it improves as you use the plugin. Skills tell you when an output used a default you should tune. The `reg-change-monitor` agent watches the regulatory feeds and flags changes against your policy library. You can re-run setup, edit the file directly, or tell a skill to record a new position.
...
Your configuration is stored at `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` and survives plugin updates — you only run setup once.
