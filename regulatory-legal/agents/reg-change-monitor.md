...
description: >
  ...
  Runs per the cadence in ~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md. Filters by materiality threshold so the
...
Per `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` → Feed configuration → Check cadence. Default weekly; daily if the regulatory environment is active.

## What it does

1. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold.
