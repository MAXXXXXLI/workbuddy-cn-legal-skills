...
1. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`. Confirm vendor governance positions are populated — if not, stop and direct to setup.
...
6. AI policy consistency diff vs. `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` → `## Vendor AI governance`. Also read `## AI policy commitments`
— vendor terms can't be consistent with a use restriction our own policy imposes if
we've agreed to something different.

If `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` contains `[PLACEHOLDER]`, surface this bounce:
...
Review each term below. For each, extract what the vendor's contract actually says and compare it against the position in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` → `## Vendor AI governance` (standard / acceptable fallback / automatic no). The default positions come from the team's playbook, not from this skill.
...
| **Stacked-vendor accountability** | Is this vendor the model provider (e.g., WorkBuddy CN adaptation, OpenAI, Google, Meta), or are they a deployer of someone else's model (e.g., a SaaS wrapper of WorkBuddy, ChatGPT, or Gemini) or a reseller of infrastructure-hosted foundation models (WorkBuddy CN adaptation-on-Bedrock, WorkBuddy-on-Vertex, OpenAI-on-Azure)? If the latter: there are TWO vendors' terms in play — the one you're reviewing, plus the upstream model provider's terms. Identify (a) whose terms govern training on inputs, retention, and safety, (b) who is contractually liable for model behavior, and (c) whether each upstream commitment (e.g., "no training on inputs") is flowed down to you, or remains between the vendor and the upstream provider only. Flag any clause where one party disclaims responsibility for the other (e.g., "WorkBuddy CN adaptation is not responsible for Bedrock or any other services it receives from AWS"; "Azure disclaims responsibility for OpenAI model outputs") and whether the counter-party's contract closes the gap. Do not review the two contracts in isolation. |

If `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` doesn't define a position for a term on this list, ask: "Your playbook doesn't cover [term]. What's your default position, your acceptable fallback, and your automatic no? I'll add it to `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` so the next review is consistent."

---

## Playbook comparison

For each term above, compare what we found to the positions in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`.
...
> **Our position:** [from `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`]
> **Gap:** [specific delta — or "Aligned"]
> **Proposed fix:** [specific redline language, or "escalate — outside fallback"]

Use the severity ratings consistently (calibrated against `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` positions):
...
- 🔴 **Critical** — outside fallback; deployment should not proceed without resolution. Escalate per `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`.
...
Cross-check the vendor's terms against our AI policy commitments in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`.
...
**Before recommending signature of a vendor AI agreement (the version the company will execute):** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
[For each 🔴 and 🟠: the fallback from `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`, or "escalate — outside fallback"
...
3. **Model provider** (e.g., WorkBuddy CN adaptation, OpenAI, Google, Meta) — the LLM
4. **Hosted knowledge base / RAG source** (e.g., a vector database, a third-party data corpus, a retrieval service) — the data WorkBuddy reads from
...
- It doesn't decide whether to accept terms outside the fallbacks. It routes those
  per the escalation table in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`.
