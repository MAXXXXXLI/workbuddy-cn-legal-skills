...
description: >
  ...
  `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`. Use when
  user says "impact assessment for", "assess this AI use case", "run an AIA", "generate an
  AIA", "we need to document this AI system", "AI risk assessment for X", or follows a
  conditional triage result. WorkBuddy 中国语境适配：默认中国大陆法域，用于AI 治理法务场景下的AI
  影响评估。中文触发词包括：中国法、中国合规、AI 治理法务、AI 影响评估、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**AI 治理法务**；当前技能：**AI 影响评估**。
...
1. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`. Confirm impact assessment house style is populated.
...
6. Policy diff against `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` AI policy commitments.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` → `## Impact assessment house style`. That has:
- What triggers an impact assessment at this company
- The structure template extracted from the seed assessment
- Typical depth
- Who signs off

If the seed structure is in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`, **use it**. The point is that this assessment
looks like the other assessments this team produces.

**Jurisdictional scope.** This assessment applies the regulatory regimes listed in `## Regulatory footprint` in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`. AI legal rules, risk classifications, and deployment obligations vary materially by jurisdiction and are moving fast. If this system is (or will be) deployed outside that footprint, or if a choice-of-law question is in play, this analysis may not apply as written — re-run or expand the footprint.

---

## Step 0: Is an impact assessment needed?

Check the trigger criteria in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`.
...
- Is the use case in the elevated or high governance tier per `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`?
...
Before intake, determine which track to run. The tier definitions and the fast-track criteria come from `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` (`## Use case registry` and `## Governance tiers`), not from any hardcoded regime-specific framework.
...
> **No silent supplement.** If a research query to the configured legal research tool (北大法宝/威科先行/官方法规库, EUR-Lex, regulator sites, or firm platform) returns few or no results for a regime's risk tiers or triggers, report what was found and stop. Do NOT fill the gap from web search or model knowledge without asking. Say: "The search returned [N] results from [tool]. Coverage appears thin for [regime / topic]. Options: (1) broaden the search query, (2) try a different research tool, (3) search the web — results will be tagged `[web search — verify]` and should be checked against the issuing authority before relying, or (4) flag as unverified and stop. Which would you like?" A lawyer decides whether to accept lower-confidence sources.
>
...
**Fast track vs. full assessment:** `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` defines what qualifies for abbreviated treatment. If `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` doesn't define fast-track criteria, default to full assessment and ask the user what criteria they want captured for next time.
...
For each regime in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` → `## Regulatory footprint` that applies to this system — **plus any regime surfaced by the re-derivation above** — research the currently operative risk classification framework and determine where the system lands.
...
**Use the seed structure from `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`.** If none was captured, use this default:
...
|---|---|---|
| [commitment from `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` AI policy section] | 🟢 / 🟡 / 🟠 / 🔴 | |
...
Regulatory citations in Section 6 (and anywhere else) were generated by an AI model and have not been verified against primary sources. Before the assessment is certified or relied on, run a verification pass against a legal research tool (北大法宝/威科先行/官方法规库, EUR-Lex, or your firm's platform) for each cited provision — confirm the pinpoint, currency, and any delegated or implementing acts. The AI regulatory landscape shifts quickly; verify before advising. Source tags on each citation (e.g., `[EUR-Lex]`, `[web search — verify]`) show where it came from; `verify` tags carry higher fabrication risk and should be checked first.
```

**Before certifying the AIA (the Sign-off step, marking Status: APPROVED):** Read `## Who's using this` in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
Every assessment should cross-check against the AI policy commitments in `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md`.
Common drift:
