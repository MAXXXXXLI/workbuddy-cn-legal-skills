...
| Skill | Purpose |
|---|---|
| **cold-start-interview** | Writes `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` from interview + seed docs |
...
| **policy-starter** | Produces a first-draft AI usage policy sourced from published model policies (ABA, 司法行政机关/律师协会s, ILTA, CLOC, NIST, EU AI Act, peer policies), adapted to your practice profile — draft for attorney review, not a finished policy |
...
Your configuration is stored at `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` and survives plugin updates.
...
- The `## Company profile` section is the first block of `~/.workbuddy/skills/config/workbuddy-cn-legal/ai-governance-legal/CLAUDE.md` by convention. If
