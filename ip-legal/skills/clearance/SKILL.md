...
description: >
  ...
  before a full professional search. This skill never concludes a mark is clear. WorkBuddy
  中国语境适配：默认中国大陆法域，用于知识产权法务场景下的商标/品牌检索初筛。中文触发词包括：中国法、中国合规、知识产权法务、商标/品牌检索初筛、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**知识产权法务**；当前技能：**商标/品牌检索初筛**。
...
5. Similar-marks search against what's connected (Solve Intelligence, 人民法院案例库/中国裁判文书网, Descrybe, or whatever MCP is available). If nothing is
...
Before running clearance, read `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`. Pull:
...
- **Integrations** from `## Available integrations` (人民法院案例库/中国裁判文书网 / Solve Intelligence / Descrybe — each determines what searches are available to run, what the fallback is, and what gets attributed in the output).
...
Read `## Available integrations` from `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`:
...
- **If a legal research connector is available** (人民法院案例库/中国裁判文书网 for litigation for case law and TTAB decisions): sweep for reported disputes involving
...
> Intelligence, Descrybe, 人民法院案例库/中国裁判文书网, state registries, Madrid/WIPO, or any
...
Per the decision posture in `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`:
...
Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` `## Outputs`.
...
TESS, Solve Intelligence, Descrybe, 人民法院案例库/中国裁判文书网, state registries,
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/matters/<matter-slug>/outputs/clearance-<mark-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/outputs/clearance-<mark-slug>-YYYY-MM-DD.md`
