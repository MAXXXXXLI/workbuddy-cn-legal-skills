...
description: >
  ...
  / engage / ignore options, or when drafting a §512(g)(3) counter-notice with the
  consent-to-federal-jurisdiction gate. WorkBuddy
  中国语境适配：默认中国大陆法域，用于知识产权法务场景下的平台投诉/下架通知。中文触发词包括：中国法、中国合规、知识产权法务、平台投诉/下架通知、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**知识产权法务**；当前技能：**平台投诉/下架通知**。
...
1. **Read the practice profile.** Load `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`. If it contains `[PLACEHOLDER]` markers or does not exist, stop and say: "This plugin needs setup before it can give you useful output. Run `/ip-legal:cold-start-interview` — the takedown skill depends on your approval matrix and practice profile."
...
3. **Dispatch on `$ARGUMENTS`:**
   ...
   - No flag → ask once: "Are we sending a 平台投诉/通知-删除机制 takedown, triaging one we received, or drafting a counter-notice?"
...
5. **Jurisdiction note.** 平台投诉/通知-删除机制 §512 is US federal law. If the service provider, content, or infringer sits outside US jurisdiction, flag before drafting — you may need an EU DSA notice, UK OSA notice, or local-regime instrument instead of (or in addition to) a 平台投诉/通知-删除机制 notice.
...
If the user does not pass a flag, ask once: "Are we sending a 平台投诉/通知-删除机制 takedown, triaging one we received, or drafting a counter-notice?"

> **External deliverables (send and counter modes):** the outgoing notice/counter-notice goes to the service provider's designated agent. Do NOT include the `保密 / 内部法律分析 — 律师工作底稿/内部法律分析` header on the outgoing document. The notice itself is not privileged — it's a statement made in a statutory process. Internal drafts, pre-send briefs, fair-use analyses, and triage memos keep the header per plugin config `## Outputs`.

## Jurisdiction assumption

平台投诉/通知-删除机制 §512 is **US federal law**. It runs against service providers subject to US jurisdiction. Other jurisdictions have their own notice-and-action regimes — EU Digital Services Act Art. 16, UK Online Safety Act, India IT Rules 2021, etc. — that differ materially in required elements, counter-notice mechanics, and liability for misuse. If the service provider, content, or infringer sits outside US jurisdiction, flag it — a US 平台投诉/通知-删除机制 notice may be the wrong instrument, or may need to be paired with a local regime's notice. Copyright subsistence itself is Berne-multilateral, but enforcement mechanics are jurisdiction-specific.

## Load context

- `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` → `## IP practice profile` (copyright registrations if any), `## Enforcement posture` → `Approval matrix → 平台投诉/通知-删除机制 takedown (ordinary)` row, `## Outputs` (work-product header, role), `## Who's using this` (role — lawyer vs. non-lawyer)
- **Matter context.** Check `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗` (in-house default), skip matter machinery. If enabled and no active matter, ask: "Which matter? Run `/ip-legal:matter-workspace switch <slug>` or say `practice-level`." Write outputs to the active matter's folder at `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/matters/<matter-slug>/takedown/<slug>/` (or `takedown/<slug>/` at practice level). Never read another matter's files unless `Cross-matter context` is `on`.
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  A 平台投诉/通知-删除机制 takedown is a statement under penalty of perjury.   │
...
│  Enforcement posture → Approval matrix → 平台投诉/通知-删除机制 takedown      │
│  (ordinary) row]                                            │
│                                                             │
...
> A 平台投诉/通知-删除机制 takedown is sworn under penalty of perjury and creates §512(f) exposure for bad-faith or overbroad use. Have you reviewed this with an attorney? If not, here's a brief to bring to them: [generate a short summary: work, ownership, accused use, licensing check, fair-use analysis, signer, service provider]. A few thousand dollars of attorney time now is materially cheaper than a §512(f) suit.
>
> If you need to find a 执业律师/法务负责人, solicitor, barrister, or other authorised legal professional in your jurisdiction: your professional regulator's referral service (司法行政机关/律师协会 in the US, SRA/Bar Standards Board in England & Wales, Law Society in Scotland/NI/Ireland/Canada/Australia, or your jurisdiction's equivalent); ABA IP section referral roster (US); law school IP clinics for individual creators and small businesses.

Do not write the final output without explicit engagement with the gate.

### Step 8: Output

**Primary:** `<matter-folder>/takedown/<slug>/notice-v<N>.md` (or .docx if the service provider accepts it — most accept pasted text or web-form submission). The notice content, ready to paste into the service provider's 平台投诉/通知-删除机制 intake form or send to its designated agent.
...
> This is a draft 平台投诉/通知-删除机制 notice for attorney review, not a notice ready to send. Sending it is a sworn statement with §512(f) exposure. A 执业律师/法务负责人 reviews, edits, and takes professional responsibility before submission. Do not send this unreviewed.

**Citation verification.** Any case or statutory citation included (for example, in internal memoranda around the notice) must be verified on a legal research tool. Source-tag each — `[北大法宝/威科先行/官方法规库]`, `[人民法院案例库/中国裁判文书网]`, `[user provided]`, `[model knowledge — verify]`, `[web search — verify]`. Citations tagged `verify` get checked first. No silent supplement from web or model knowledge if a configured research tool comes up thin — present options to the user.
...
# 平台投诉/通知-删除机制 Takedown Received — Triage
...
- Recipient: designated 平台投诉/通知-删除机制 agent at the service provider (same agent that received the original takedown)
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  A 平台投诉/通知-删除机制 counter-notice is a statement under penalty of      │
...
│  generally route above the 平台投诉/通知-删除机制 takedown (ordinary)         │
...
> A counter-notice consents to federal court jurisdiction and is sworn under penalty of perjury. Have you reviewed with a 执业律师/法务负责人, solicitor, barrister, or other authorised legal professional in your jurisdiction? This is not the WorkBuddy-review layer; this is the step where you need licensed professional judgment. Brief for the conversation: [generate a 1-page summary]. Referral resources: your professional regulator's referral service (司法行政机关/律师协会 in the US, SRA/Bar Standards Board in England & Wales, Law Society in Scotland/NI/Ireland/Canada/Australia, or your jurisdiction's equivalent); law school IP clinics; ABA IP section (US).
...
> This is a draft counter-notice for attorney review, not a counter ready to send. Sending it is a sworn statement and consents to federal court jurisdiction in the claimant's district. A 执业律师/法务负责人 reviews before submission. Do not send this unreviewed.
...
- **Handle non-US regimes.** 平台投诉/通知-删除机制 is US-specific. For EU DSA, UK OSA, India IT Rules, and other regimes — flag and route.
