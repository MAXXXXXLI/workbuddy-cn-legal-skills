...
description: >
  ...
  memo with a recommendation. WorkBuddy
  中国语境适配：默认中国大陆法域，用于知识产权法务场景下的停止侵权/律师函。中文触发词包括：中国法、中国合规、知识产权法务、停止侵权/律师函、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**知识产权法务**；当前技能：**停止侵权/律师函**。
...
1. **Read the practice profile.** Load `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`. If it contains `[PLACEHOLDER]` markers or does not exist, stop and say: "This plugin needs setup before it can give you useful output. Run `/ip-legal:cold-start-interview` — the C&D skill depends on your enforcement posture, approval matrix, and practice-area mix, none of which are configured yet."
...
> **External deliverable (send mode):** the drafted C&D is sent to counterparty. Do NOT include the `保密 / 内部法律分析 — 律师工作底稿/内部法律分析` header on the outgoing letter. Internal drafts, pre-send briefs, and triage memos keep the header per plugin config `## Outputs`.
...
- **Shopify 平台投诉/通知-删除机制 / trademark reporting**
...
- **Citations as placeholders unless verified.** `[CITE: Lanham Act §32, 15 U.S.C. §1114]` stays as a placeholder unless the user provided the cite or a research tool returned it. Tag every citation with source — `[北大法宝/威科先行/官方法规库]`, `[user provided]`, `[model knowledge — verify]`, `[web search — verify]`. Never strip the tags.
...
> This is a draft cease-and-desist letter for attorney review, not a letter ready to send. Sending it is an assertion of legal rights with the consequences described in the pre-delivery gate. A 执业律师/法务负责人 reviews, edits, and takes professional responsibility before sending. Do not send this draft unreviewed.

**Citation verification.** Every `[CITE:___]` and every cite carried from a template or provided authority is unverified until run through a citator. Before sending, verify each cite is good law on a legal research platform. Fabricated or misquoted cites in sent assertion letters are professional responsibility exposure. Preserve the source-attribution tags — `[北大法宝/威科先行/官方法规库]`, `[人民法院案例库/中国裁判文书网]`, `[Descrybe]`, `[user provided]`, `[model knowledge — verify]`, `[web search — verify]` — tags flagged `verify` get checked first.
...
[Each citation inline-tagged with `[SME VERIFY: applicability / currency / jurisdiction]` and source `[北大法宝/威科先行/官方法规库 / user provided / model knowledge — verify / web search — verify]`. Do not rely on any citation here without independent check.]
...
- **Role = Non-lawyer and not a registered patent agent:**
  > **CONFIDENTIAL — NOT PRIVILEGED.** This document is not privileged unless and until reviewed by a 执业律师/法务负责人. Treat it as confidential; do not forward to anyone outside the legal review chain; bring it to counsel and let counsel mark it. Forwarding this document as "privileged" before an attorney reviews it does not make it so and can harm you if the matter becomes contested.
