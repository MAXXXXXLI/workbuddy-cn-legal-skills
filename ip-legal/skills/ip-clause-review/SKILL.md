...
description: >
  ...
  scope, or when an agreement with IP provisions is pasted or attached. WorkBuddy
  中国语境适配：默认中国大陆法域，用于知识产权法务场景下的知识产权条款审查。中文触发词包括：中国法、中国合规、知识产权法务、知识产权条款审查、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**知识产权法务**；当前技能：**知识产权条款审查**。
...
Reviews the IP clauses in an agreement against the practice profile in `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`. Flags assignment gaps, ownership ambiguity, license-scope issues, and IP warranty/indemnity problems. Produces a memo with per-clause findings, prioritized by risk, with suggested redline language where appropriate.

## Instructions

1. **Load `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`.** If placeholders present, stop and prompt: "Run `/ip-legal:cold-start-interview` first — I need to learn your practice profile before I can review IP clauses against it."
...
**Before reading the agreement, read `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`.** If it is missing or still contains placeholders, stop and run `/ip-legal:cold-start-interview`. The practice profile tells you:
...
**Escalation:** Per `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`, assignment-scope gaps escalate to [approver].
```
...
Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`).
