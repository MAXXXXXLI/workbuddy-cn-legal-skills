...
description: >
  ...
  be open-sourced. WorkBuddy
  中国语境适配：默认中国大陆法域，用于知识产权法务场景下的开源合规审查。中文触发词包括：中国法、中国合规、知识产权法务、开源合规审查、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**知识产权法务**；当前技能：**开源合规审查**。
...
**Before scanning dependencies, read `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`.** If it is missing or still contains placeholders, stop and run `/ip-legal:cold-start-interview`. The practice profile tells you:
...
Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` → `## Outputs` (differs by user role — see `## Who's using this`).
