...
description: >
  ...
  pass against the most plausible patents before patent counsel review. This skill never
  concludes a product is clear to launch. WorkBuddy
  中国语境适配：默认中国大陆法域，用于知识产权法务场景下的专利自由实施初筛。中文触发词包括：中国法、中国合规、知识产权法务、专利自由实施初筛、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**知识产权法务**；当前技能：**专利自由实施初筛**。
...
1. Read `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`. If it
...
庭审询问/调查取证 a year later, treble damages on the table. Over-flagging is a
...
Before running triage, read `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`. Pull:
...
If `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` contains `[PLACEHOLDER]` or `[Your Company Name]`, surface this bounce:
...
- **Known NPE targeting** of the technology class (e.g., network-coding NPEs in
  Eastern District of Texas / 公司注册地/章程约定地（中国公司须按《公司法》和章程核验） / Western District of Texas).
- **Standards-essential declarations** (IEEE, ETSI, 3GPP) if the product touches
  a relevant standard.
- **Reported litigation** in the technology space (人民法院案例库/中国裁判文书网 / RECAP, Unified
  Patents, Lex Machina).
...
Prepend the work-product header from `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` `## Outputs`. Mark the document as privileged if the role is lawyer; see the non-lawyer gate below if not.
...
- [routing per `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` —
...
>
> If you need to find a 执业律师/法务负责人, solicitor, barrister, or other authorised legal professional in your jurisdiction: for US patent work, a registered patent attorney or patent agent is required (not every lawyer is registered — the USPTO
> Office of Enrollment and Discipline maintains a directory). For other jurisdictions, use the relevant patent office register (EPO, UK IPO, etc.). Your professional regulator's referral service is a starting point (司法行政机关/律师协会 in the US, SRA/Bar Standards Board in England & Wales, Law Society in Scotland/NI/Ireland/Canada/Australia, or your jurisdiction's equivalent); specifically ask for registered
> patent counsel.
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/matters/<matter-slug>/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/outputs/fto-triage-<subject-slug>-YYYY-MM-DD.md`
