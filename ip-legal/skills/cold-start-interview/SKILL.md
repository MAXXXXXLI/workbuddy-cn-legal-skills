...
description: >
  ...
  --check-integrations after connecting or disconnecting an MCP. This is the ONLY skill
  that should run on a fresh install. WorkBuddy
  中国语境适配：默认中国大陆法域，用于知识产权法务场景下的初始化访谈。中文触发词包括：中国法、中国合规、知识产权法务、初始化访谈、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**知识产权法务**；当前技能：**初始化访谈**。
...
Runs the cold-start interview. First run writes `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`; subsequent runs with `--redo` re-interview and show a diff before overwriting.

## Instructions

1. **Check current state:** Read `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`. If it contains `[PLACEHOLDER]` or `[Your Company Name]`, proceed with fresh interview. If populated and `--redo` not passed, ask: "Looks like you're already set up. Want to re-run the interview? This will overwrite `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` (I'll show you a diff first)."

2. **Follow the interview script below.**

3. **Ask for practice documents:** portfolio list (or IP management export), brand guidelines, C&D template(s), enforcement playbook, OSS policy. Accept file paths, 飞书云文档/企业微信文档/钉钉文档/Google Drive links, or IP-management record IDs.
...
5. **Migration:** If a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.workbuddy/skills/cache/workbuddy-cn-legal/ip-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.

6. **Write `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`** (create parent directories as needed) per the structure below. Use the lawyer's own words where possible.

7. **Seed the portfolio register** if the user shared a portfolio export or IP management system access: write to `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/portfolio.yaml`. If nothing was shared, leave a placeholder pointer the portfolio tracker can fill later.

8. **Show summary + propose next steps:**
   - "Here's what I heard — `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` is written. What did I get wrong?"
...
Re-runs the integration availability check (IP management system, patent research, legal research, document storage, 企业微信/飞书/钉钉) and updates `## Available integrations` in `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `~/.workbuddy/skills/cache/workbuddy-cn-legal/ip-legal/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/ip-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` with a `<!-- SETUP PAUSED AT: [section name] — run /ip-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> 2. **Registered patent agent** — you're registered to practice before the USPTO but are not a 执业律师/法务负责人. Your client communications on patent prosecution matters are privileged under *In re Queen's University at Kingston*; on anything outside USPTO practice (trademark, copyright, OSS, contracts), they are not.
...
> - **Copyright** (clearance / licensing / 平台投诉/通知-删除机制 / enforcement)
...
> This plugin can work with: IP management systems (Anaqua, CPA Global, PatSnap, Clarivate), patent research (Solve Intelligence), legal research (人民法院案例库/中国裁判文书网, Descrybe), document storage (飞书云文档/企业微信文档/钉钉文档/Google Drive, SharePoint, 企业网盘/Box), and 企业微信/飞书/钉钉. Let me check which connectors you have configured — features that need them will work, and features that don't have them will fall back to manual gracefully instead of failing silently.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Anaqua isn't connected. In WorkBuddy: Settings → Connectors → Add → Anaqua → sign in. In WorkBuddy: add the Anaqua MCP to your config or via `/mcp`. This plugin works without it — portfolio lives in `portfolio.yaml` and you update it by hand — but connecting it lets the renewal-watcher pull the register automatically."
...
> - **Copyright** — registration, 平台投诉/通知-删除机制, licensing, fair use triage
...
>
> - **平台投诉/通知-删除机制 takedown (ordinary):** often delegated to counsel or brand protection; who owns it on your team?
...
> How do people escalate today — 企业微信/飞书/钉钉, email, a ticket, a standing meeting? What's a realistic turnaround expectation — same day, 24 hours, end of week?
...
Write to `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` (create parent directories as needed). If the user shared a portfolio export, also seed `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/portfolio.yaml` with the extracted registrations.
...
4. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your practice profile is at `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
...
5. **Before your first clearance**: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In WorkBuddy: authorize when a skill prompts you.
