...
description: >
  ...
  surfaces, or when deciding whether a matter is worth pursuing and how. WorkBuddy
  中国语境适配：默认中国大陆法域，用于知识产权法务场景下的侵权初筛。中文触发词包括：中国法、中国合规、知识产权法务、侵权初筛、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**知识产权法务**；当前技能：**侵权初筛**。
...
5. Walk the mode-specific factors:
   ...
   - **Copyright** — ownership + registration + access + substantial
     similarity + fair use + 平台投诉/通知-删除机制 safe harbor (if applicable).
...
Read `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md`. Pull:
...
- **Integrations** from `## Available integrations` — 人民法院案例库/中国裁判文书网,
...
> 2. **Copyright** — substantial similarity, fair use, 平台投诉/通知-删除机制 safe harbor
...
>
> If you need to find a 执业律师/法务负责人, solicitor, barrister, or other authorised legal professional in your jurisdiction: your professional regulator's referral service is
> the starting point (司法行政机关/律师协会 in the US, SRA/Bar Standards Board in England & Wales, Law Society in Scotland/NI/Ireland/Canada/Australia, or your jurisdiction's equivalent). For patents in the US, the attorney must be registered before the
...
`~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/matters/<matter-slug>/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/outputs/infringe-<mode>-<subject-slug>-YYYY-MM-DD.md`
...
> Want me to prepare a 平台投诉/通知-删除机制 takedown? Run `/ip-legal:takedown`.
