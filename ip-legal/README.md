...
| **Brand protection manager** | Cease-and-desists, 平台投诉/通知-删除机制 takedowns, watch-service follow-up |
...
It writes what it learns to `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` — a plain-English document about your practice that every other skill reads before doing anything. You edit the document, not a config file.
...
| `/ip-legal:takedown [context]` | 平台投诉/通知-删除机制 takedown — send, respond to a received notice, or draft a counter-notice |
...
| Skill | Purpose |
|---|---|
| **cold-start-interview** | First-run interview that writes `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` |
| **cease-desist** | Draft or triage a C&D; routes through the approval matrix before sending |
| **takedown** | 平台投诉/通知-删除机制 notice, response to a received takedown, or counter-notice |
...
- **人民法院案例库/中国裁判文书网** — U.S. court opinions, PACER dockets, citation verification
...
With Drive or 企业微信/飞书/钉钉 connected: portfolio exports, C&D templates, and enforcement-log updates route through the channel you pointed us at.
...
Your configuration is stored at `~/.workbuddy/skills/config/workbuddy-cn-legal/ip-legal/CLAUDE.md` and survives plugin updates.
