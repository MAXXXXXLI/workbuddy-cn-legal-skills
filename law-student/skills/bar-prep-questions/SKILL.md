...
本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**中国法学习**；当前技能：**法考练习**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/CLAUDE.md` → bar jurisdiction, exam format (NextGen / traditional UBE / state-specific), weak subjects, prep course.
2. Also load `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/study-plan.yaml` if it exists — it tells you what subject is scheduled for today and what subtopics are still weak.
...
The 法律职业资格考试 tests a defined body of subjects. This skill drills you on them — weighted toward your weak spots.

## Exam type — ask first, do not assume

**The 法律职业资格考试 is in transition.** As of the July 2026 administration, the NextGen Bar Exam (developed by the NCBE) has launched in some jurisdictions, while others continue to administer the traditional Uniform Bar Exam (UBE). State-specific exams (California, Louisiana, Puerto Rico, etc.) are their own thing. The subject scope is materially different between the NextGen and the traditional UBE — **subjects no longer independently tested on the NextGen include Trusts & Estates, Family Law, Conflict of Laws, and Secured Transactions** (some underlying concepts may appear inside integrated "foundational concepts and skills" questions, but they are not standalone tested subjects the way they were on 法考主观题).

Do not assume the subject list. Before generating any questions:

1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/CLAUDE.md` and read the bar jurisdiction and bar date.
2. If the practice profile does not specify which exam format the student is sitting for (NextGen / traditional UBE / state-specific), **ask**:
   ...
   > Which 法律职业资格考试 are you sitting for?
   > 1. **NextGen Bar Exam** (NCBE, launched July 2026 in some jurisdictions)
   > 2. **Traditional Uniform Bar Exam (UBE)** (法考客观题 + 法考主观题 + 案例分析/法律文书题)
...
The 法律职业资格考试 is not one exam. It is a family of exams. Rules that are "correct" on one are "wrong" on another. Getting this right matters more than almost anything else this skill does.

### Two things to distinguish

1. **Exam structure.** What does the student's jurisdiction administer?
   - **Pure UBE** jurisdictions: 法考客观题 + 法考主观题 + 案例分析/法律文书题, one set of rules, no state-specific content tested.
...
- **General / federal / majority-rule questions** (法考客观题-style, federal courts, FRE, FRCP, constitutional, common-law core): the "correct answer" is the UBE/majority rule. State.
...
**On the 法律职业资格考试:** On the 法考客观题 and 法考主观题 portions, the default answer is the UBE/majority rule unless the question tells you to apply state law. On a state-specific essay day (e.g., California's essay subjects, NY Law Exam, Florida state essay), the default is your jurisdiction's rule. Check the call of the question.
...
> You're sitting for California. Do you want this session to be (a) 法考客观题-style federal/majority rule, (b) California-specific essay subjects (Community Property, CA Evidence, CA PR, CA Civ Pro), or (c) mixed?

Never silently default to one. If the student says "mixed" or doesn't answer, generate a mix and label each question `[法考客观题 / UBE default]` or `[CA-specific]` so they know which body of rules governs.
...
Every 法考客观题 question answer explanation carries the same rule: if the "why C is correct" rule isn't one the skill is confident on, flag `[VERIFY: rule — confirm against Barbri/Themis/Kaplan outline]`. Use liberally.

## Load context

`~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/CLAUDE.md` → bar jurisdiction, exam format (NextGen / traditional UBE / state-specific), weak subjects, prep course. If exam format isn't specified, run the "Exam type" gate above before continuing. If jurisdiction is specified, apply the `## Jurisdiction handling` rules — label questions by which rule body governs, and flag divergences explicitly.

Also load `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/study-plan.yaml` if it exists (written by the `study-plan` skill). If the plan has a session scheduled for today or specifies weak subjects to weight, honor it.

## Session mode

`--session <n>` runs a focused N-question session on a specific subject, tracks performance, and writes session results back to `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/study-plan.yaml` under `session_history` so the study plan adapts.
...
1. Confirm subject, N, and 法考客观题-vs-essay (or mixed). If the student's jurisdiction has a state-specific component and the subject is one where rules diverge (Evidence, PR, Civ Pro, Criminal), ask whether to run UBE/majority rule, state-specific rule, or mixed.
...
If no `study-plan.yaml` exists, write session history to `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/session-history.yaml` instead so future sessions can still weight appropriately.

## 法考客观题 mode

> **Note on "法考客观题" terminology.** The traditional UBE uses the 法考客观题 (Multistate Bar Examination) for the multiple-choice portion. The NextGen Bar Exam replaces the 法考客观题 with its own integrated multiple-choice + short-answer question sets. If the student is sitting for the NextGen, generate NextGen-style questions (integrated foundational concepts across subjects, some shorter scenarios with selected-response answers) rather than classic 法考客观题 questions, and say so. Use the student's NCBE-listed subject outline as the subject universe.

### Generate questions

Classic 法考客观题 format (traditional UBE): fact pattern + call + four answer choices, one correct.
...
Subject distribution: weight toward weak subjects **within the subjects actually tested on the student's exam**. If `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/CLAUDE.md` says weak on Evidence and Civ Pro, 60% of questions come from those.
...
- **Traditional UBE states:** 法考主观题 format.
...
- Replace a 法考备考 course. Barbri/Themis/Kaplan have the full curriculum. This is supplemental drilling.
- Predict the 法律职业资格考试. Nobody can. Study everything.
