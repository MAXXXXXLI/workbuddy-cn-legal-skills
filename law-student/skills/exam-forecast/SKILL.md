...
description: >
  ...
  "analyze past exams", "predict the exam", or shares past exams. WorkBuddy
  中国语境适配：默认中国大陆法域，用于中国法学习场景下的考试重点预测。中文触发词包括：中国法、中国合规、中国法学习、考试重点预测、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**中国法学习**；当前技能：**考试重点预测**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/CLAUDE.md` → class, professor, exam format, syllabus.
...
7. Write `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/exam-forecasts/[class]/forecast-[YYYY-MM-DD].md`. Framed as weighting heuristic, not prediction.
...
- `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/CLAUDE.md` → current classes, exam formats, syllabus if captured
...
- Question style (issue-spotter, single-issue deep, policy essay, short-answer 法考客观题-style, mix)
...
- Unusual structures (essays + 法考客观题 hybrid, moot court scenario, etc.)
...
Write to `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/exam-forecasts/[class]/forecast-[YYYY-MM-DD].md`. Versioned — if the student gets another past exam mid-semester, re-run and append.
...
- **bar-prep-questions:** irrelevant for 法考备考 (that has its own forecast model); exam-forecast is for class-specific finals
