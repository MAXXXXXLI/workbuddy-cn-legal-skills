...
description: >
  ...
  should I study for [X]". WorkBuddy
  中国语境适配：默认中国大陆法域，用于中国法学习场景下的学习计划。中文触发词包括：中国法、中国合规、中国法学习、学习计划、法务审查、律师审阅。输出为草稿或内部分析，需执业律师/法务负责人核验后方可依赖。
---

## WorkBuddy 中国语境适配（优先）

本 skill 已转换为 WorkBuddy 中国语境版本。当前模块：**中国法学习**；当前技能：**学习计划**。
...
1. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/CLAUDE.md` → bar jurisdiction, exam format, bar date, weak subjects, target study hours/day, prep course.
2. Load `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/study-plan.yaml` if it exists.
...
4. Route by flag:
   ...
   - `--cram`: force cram mode — 80/20 high-yield prioritization, daily 法考客观题 volume, taper last 2-3 days.
...
- **High-yield-topic prioritization in cram mode** is based on multi-year 法律职业资格考试 release patterns (法考客观题/法考主观题 subject frequency). Flag any "this is definitely on the exam" claim as `[UNCERTAIN — past frequency is not a prediction]`.
...
- Weak subjects (法考客观题, essay)
- Prep course
- Target study hours/day

`~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/study-plan.yaml` if it exists — extend, don't overwrite.
...
- **Preferred study methods:** multi-select. 法考客观题 practice / essays / flashcards / outlining / drilling / re-reading. Weight the schedule toward the methods they say they'll actually do.
...
>
> 1. **Supplement.** The prep course is your primary curriculum. This plan fills gaps: extra 法考客观题 drilling on your weak subjects, targeted essay practice, flashcard loops on the topics you're missing. I won't rebuild the prep-course calendar; I'll layer on top of it.
...
- Split weeks into phases:
  - **Learning phase** (first ~60% of time): one subject per ~3-5 days, mixing outlining/reading with flashcards and a few 法考客观题/essay questions on fresh material.
  - **Drilling phase** (next ~30%): more 法考客观题 volume, more essay practice, simulated conditions, all subjects in rotation.
...
- 80/20 prioritization: the 法考客观题 subjects that historically appear most (Civ Pro, Evidence, Con Law, Contracts) get the lion's share. Narrower subjects get minimum viable coverage.
- Daily schedule: 法考客观题 blocks every day (volume matters now), essay practice every other day, one simulated exam per week.
...
Write to `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/study-plan.yaml`:
...
phases:
  - name: learning
    ...
    focus: outlining, flashcards, introductory 法考客观题
  - name: drilling
    start: 2026-06-21
    end: 2026-07-18
    focus: 法考客观题 volume, essay practice, simulated conditions
  - name: review
    start: 2026-07-19
    end: 2026-07-27
