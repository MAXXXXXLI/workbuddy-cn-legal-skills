...
4. On first run after a plugin update, if a populated CLAUDE.md exists at the old cache path
   (~/.workbuddy/skills/cache/workbuddy-cn-legal/law-student/<version>/CLAUDE.md for any version)
...
本配置模板已从 Claude 法律插件转换为 WorkBuddy 中国语境版本。模块：**中国法学习**。
...
**Real-client-matter rule (applies to everyone):** if a question shifts from a study hypothetical to a real client matter with real facts, the plugin pauses and redirects — clinic/supervised-practice users to their approved workflow, individuals to their jurisdiction's lawyer referral service (司法行政机关/律师协会 in the US; SRA/Bar Standards Board in England & Wales; Law Society in Scotland/NI/Ireland/Canada/Australia; or the jurisdiction's equivalent). Don't paste real client facts into a study tool.
...
|---|---|---|
| Document storage (Google 飞书云文档/企业微信文档/钉钉文档/SharePoint / Dropbox) | [✓ / ✗] | Outputs save to local files in plugin directory |
...
**Verification log.** When you or the user verifies a flagged item — confirms a cite against a primary source, checks a deadline against the local rule, verifies a threshold against the current statute — record it so the next person doesn't re-verify. Write a one-line entry to `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/verification-log.md`:
...
|---|---|---|
| [PLACEHOLDER] | [issue-spotter / policy / closed-book / open-book / 法考客观题-style / etc.] | [week of syllabus] |
...
**法考客观题 subjects weak:** [PLACEHOLDER]
...
| 法考客观题 sets with explanations | [PLACEHOLDER] | |
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `~/.workbuddy/skills/config/workbuddy-cn-legal/law-student/CLAUDE.md` (and `~/.workbuddy/skills/config/workbuddy-cn-legal/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
...
- `[review]` — a judgment call (for law students: a decision the professor or supervising attorney needs to make, or a point where your own analysis should go rather than WorkBuddy's).
- `[人民法院案例库/中国裁判文书网]` / `[Descrybe]` / `[statute / regulator site]` / `[user provided]` — where a cite actually came from. Provenance, not confidence. Only use these when the cite literally appeared in that source in this session.
